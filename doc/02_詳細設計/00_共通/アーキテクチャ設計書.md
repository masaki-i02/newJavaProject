...
| 作成日 | 2026-09-01 |
| ステータス | ドラフト |
| 改訂 | 0.2（2026-09-01）BR-11 の責務分割を明確化。0.3（同日）第 2 回レビューを反映し、コンテキスト間の循環依存を 2 件解消。0.4（2026-09-08）10 章の未決事項 3 件を実装の実際に合わせて決着させた（問い合わせの書き方・監査証跡の記録者・API 型の生成） |
...
| `leave` | 年次有給休暇 | 付与、出勤率、取得の申請、時効 |
| `payroll` | 給与連携 | 給与へ渡す 1 行、出力の記録 |
...
**`application` を認めているのは、手順を写すほうが害が大きいからである。**
複数のリポジトリを束ねる操作は `domain` に置けない。
たとえば「打刻から日次勤怠を作り直す」は打刻の読み出し・就業規則の解決・保存を伴うので、
`attendance` の `application` にしか置けない。
呼ばずに `approval` 側へ書き写すと、丸めや勤務日の扱いを直したときに
**片方だけが古くなる**（[CLAUDE.md 落とし穴 67](../../../CLAUDE.md)）。

現時点で越境しているのは次の 3 か所で、いずれも図にある `approval → attendance` の辺に沿う。

| 呼ぶ側 | 呼ばれる側 | 何のために |
| --- | --- | --- |
| `CorrectionRequestService` | `TimeClockService.recalculate` | 訂正の承認で日次を作り直す |
| `CorrectionRequestService` | `MonthlySettlementService.settle` | 訂正の承認で月次を作り直す |
| `MonthlyAttendanceService` | `MonthlySettlementService.settle` | 提出を契機に月次を作り直す |
| `PaidLeaveRequestService` | `MonthlySettlementService.settle` | 年休の承認・取下げで月次を作り直す |
| `PaidLeaveRequestService` | `MonthlyAttendanceService.revertByLeave` | 年休の承認で提出済みの月を下書きへ戻す |

> **`payroll` はどこからも参照されない末端のコンテキストである。**
> 出力する 1 行は `employee`・`attendance`・`approval`・`workrule` の値を束ねたものなので、
> 依存は外へ出るだけで、逆向きの辺が要らない。
> `attendance` に置くと、04 が決めた「社員番号・氏名を返さない」を破ることになる
> （[給与連携 ドメインモデル設計書 1.1](../07_給与連携/ドメインモデル設計書.md)）。

**逆向き（図に無い辺）は `application` でも認めない。**
その場合はポートを `shared/domain` に置く（次節）。
...
| `attendance` が「この日は年休か」を知りたい | `attendance → leave` | **無い**（図は `leave → attendance`） |
...
年休も同じ形である。所有者は `leave` のままで、`attendance` はポートだけを知る。

```java
// shared/domain --- 年休の取得日を問い合わせる窓口
public interface PaidLeaveDays {
    Set<LocalDate> approvedOn(EmployeeId employeeId, DateRange period);
}

// leave/infrastructure --- 実装
...
│   │                                PremiumType（割増区分の語彙）, DomainException（例外の基底）,
│   │                                MonthClosureQuery（締め状態を問い合わせるポート）
│   ├── infrastructure/              ClockConfig（Clock の Bean 定義）
...
│   │                                NightWindow（enum）, DayType, CompanyCalendar
│   ├── application/
│   ├── infrastructure/
│   └── presentation/
├── attendance/
...
| 規約 | 理由 |
| --- | --- |
| Spring、JPA、Jackson、Hibernate に依存しない | フレームワークを起動せずに単体テストするため |
...
#### 問い合わせの書き方（未決事項 #1 の決定）

**既定は Spring Data の導出クエリと JPQL。ネイティブクエリは例外である。**

| 使うもの | 対象 | 理由 |
| --- | --- | --- |
| 導出クエリ / JPQL | ほぼすべて | エンティティ名と項目名で書くので、名前を変えるとコンパイルではなく起動時に落ちる（実行時まで気づかない SQL 文字列より早い）。永続化コンテキストの自動フラッシュが効く |
| ネイティブクエリ | **JPQL に無い SQL の機能を使うときだけ** | `WITH RECURSIVE`（部署ツリーの祖先・子孫）が該当する。再帰共通表式は JPQL に無い |
| `JdbcTemplate` | **追記専用・まるごと入れ替えの表** | 打刻・日次勤怠の内訳・監査証跡。同一性の管理も変更の追跡も要らない（[ADR 0002](../../05_ADR/0002_ドメインモデルとJPAエンティティを分離する.md)）|

> **書き込みの直後に読む問い合わせは、ネイティブクエリにしない。**
> JPA は JPQL の実行前に永続化コンテキストをフラッシュすることを求めているが、
> ネイティブクエリについては、同期すべき表を明示しないかぎり求めていない。
> 給与出力の「使った分母が動いていないか」の検査は、
> **書き込んだあとに数え直して記録と突き合わせる**（落とし穴 135）ので、
> ここを取り違えると**検査が変更前の行を数える**。
...
振る舞いを変えられるようにする。**文言では分岐させない。**
メッセージを直した瞬間に画面が壊れる。

この URN は `shared.domain.DomainException#errorCode()` が返す。
**業務エラーはすべて `DomainException` を継承する。**
`@RestControllerAdvice` はこの 1 つを捕まえて変換するので、
継承し忘れた例外は 500 になって漏れる。

実装の不備（null・桁あふれ・ありえない状態）は `DomainException` にしない。
`IllegalArgumentException` / `IllegalStateException` のままにして、
業務エラーとして利用者に見せない。

判定の基準は **「利用者の入力で踏めるか」** である。
人事が就業規則登録画面から「割増率 0.20」「法定労働時間 12 時間」を入力すれば起きる誤りは、
実装の不備ではなく業務エラーである。

```
DomainException（abstract・errorCode()）
├── BusinessRuleViolationException      法令・就業規則の定めに反する値（422）
│                                       rule() で BR-xx を返す
└── TimeClockSequenceException（abstract）
    ├── InvalidTimeClockSequenceException     並びが状態機械に反する（422・訂正申請へ）
    └── IncompleteTimeClockSequenceException  まだ退勤していない（409・退勤を打てば解消）
```

打刻の例外を 2 つに分けているのは、**画面が案内を出し分けられるようにするため**である。
1 つにまとめると、退勤し忘れた社員に「訂正申請をしてください」と案内することになる。

**同じ `errorCode` を返すクラスを 2 つ作らない。**
`month-already-closed` は `attendance`・`workrule`・`employee`・`shared` の
**4 か所**が同じ `errorCode()` と `kind()` を返すクラスを持っていた。
画面は `type` で分岐するので、どれか 1 つの `kind()` を `RULE_VIOLATION` に変えれば
**その経路だけが 422 を返す**ようになる。
`check-error-codes.py` は型の名前を突き合わせるが、
**同じ型に別の HTTP が割り当たったことは検出できない**（落とし穴 124）。

契約（`errorCode` と `kind`）は 1 つのクラスに置き、
利用者への説明（`title` と本文）だけを静的ファクトリで分ける。

```java
MonthAlreadyClosedException.of(month)                      // 意思表示（訂正申請・年休）
MonthAlreadyClosedException.recalculating(month)           // 勤怠の再計算
MonthAlreadyClosedException.affecting(month, "会社カレンダー")  // 全社共有の表
MonthAlreadyClosedException.goingBackTo(month, "異動")       // 遡る変更
```

**`String` を 2 つ並べたコンストラクタは公開しない。**
入れ替えてもコンパイルが通り、見出しと本文が入れ替わるだけで画面は動き続ける
（落とし穴 187・188）。

#### 6.2.1 業務に届く前の誤りも、こちらで変換する

**Spring MVC が投げる例外を既定のままにしない。**
必須パラメータの欠落・URL の綴りの誤り・使えない HTTP メソッドは、
既定では応答本文を持たないまま `/error` へ内部転送される。
その転送を Spring Security の `anyRequest().denyAll()` が拒むので、
**利用者には本文の無い 403 として届く。**
「権限が無い」と「要求が間違っている」は利用者にとってまったく違う話であり、
取り違えると原因に辿り着けない。

| 例外 | HTTP | `type` |
| --- | --- | --- |
| `MissingServletRequestParameterException` | 400 | `validation-failed` |
| `MethodArgumentTypeMismatchException` | 400 | `validation-failed` |
| `HttpMessageNotReadableException` | 400 | `validation-failed` |
| `MethodArgumentNotValidException` | 400 | `validation-failed` |
| `NoResourceFoundException` | 404 | `resource-not-found` |
| `HttpRequestMethodNotSupportedException` | 405 | `method-not-allowed` |

**型の不一致を実装の不備として扱わない。**
`UUID.fromString` も `LocalDate.parse` も `IllegalArgumentException` 系を投げるので、
実装の不備を拾うハンドラが先に捕まえて**理由の載らない 500** になっていた
（CLAUDE.md 落とし穴 105）。送った値が悪いことは利用者にしか直せない。

**受け取った値そのものを応答に載せない。** 送った本人は自分が送った値を知っている。
載せると、そのまま画面へ出す実装で反射型 XSS の材料になる。

あわせて Spring Security 側で **`DispatcherType.ERROR` の転送を通す。**
上の変換で拾えなかった経路（フィルタの中で起きた失敗など）まで
本文の無い 403 になることを防ぐ最後の防波堤である。
通すのは元の要求が済んだあとの内部転送だけで、新しい入口を開けるものではない。

検査は IT-API-13〜16。**ブラウザから通して初めて出た欠陥である。**
MockMvc は `/error` への転送を再現しないので、
1000 件を超える API のテストが 1 件も落ちなかった。
...
Bean は `shared/infrastructure/ClockConfig` が `Clock.system(BusinessZone.ID)` で定義する。
システム既定のゾーンを使うと、CI のコンテナ（多くは UTC）で「今日」がずれる。

`BusinessZone` は**設定値にしない**。あとから変えられると、
過去に記録した打刻の解釈まで遡って変わってしまう。
...
**監査証跡はアプリケーションが書く。DB トリガでは書かない。**

`approval_events` / `paid_leave_request_events` / `payroll_exports` は、
`infrastructure` の各アダプタが `JdbcTemplate` で `INSERT` する。

| 理由 | 内容 |
| --- | --- |
| 依頼者を知っているのはアプリケーションだけ | `Requester` は引数で渡す（`SecurityContextHolder` を読まない）。トリガから使うには `SET LOCAL` などで DB へ写す必要があり、**「誰の依頼か」の出どころが 2 か所**になる |
| 遷移の**理由**は行の差分に現れない | 提出済みから下書きへ戻る事象は、原因が訂正の承認か年休の承認かで `REVERT_BY_CORRECTION` / `REVERT_BY_LEAVE` に分かれる。トリガは前後の行しか見えないので、**何が原因で戻ったのかを証跡に残せない** |
| トリガの `RAISE` は制約違反ではない | `DataIntegrityViolationException` にならないので Problem Details へ写らず、業務エラーが 500 になる（落とし穴 81）|

トリガに残すのは、**業務上の判断を含まない機械的な事実だけ**である。

| トリガ | 役割 |
| --- | --- |
| `〜_set_updated_at` | `updated_at` に DB の `now()` を入れる（監査のため偽装できないようにする）|
| `departments_reject_cycle_trigger` | 部署ツリーの循環を最後の防波堤として拒む（業務エラーの検出は `application` 層で行う）|
...
| ID | ルール | 意図 |
| --- | --- | --- |
| AR-01 | `..domain..` は Spring / JPA / Jackson / Hibernate に依存しない | ドメインをフレームワークから独立させる |
...
| AR-06 | コンテキストは他コンテキストの `infrastructure` / `presentation` を参照しない（`domain` と `application` は 4 章の向きに限り可） | 業務の境界を守る |
...
| AR-10 | **`shared` はどの業務コンテキストにも依存しない** | `shared` が個別のコンテキストを知ると、すべてのコンテキストが間接的に結合する |

> **コンテキストを増やしたら AR-10 の禁止先も増やす。**
> `leave` を足すのに AR-10 を直さないと、そのコンテキストだけ検査対象から外れる。
> `PaidLeaveDays` のように `shared` へポートを置く場合、
> **引数にうっかり `leave` の型を取る**変更が最も起きやすいのがまさにその場所である
> （落とし穴 84・86）。禁止先と違反クラスは**同じコミットで**足す。
...
| # | 内容 | 決定時期 |
| --- | --- | --- |
| 1 | ~~複雑な集計クエリを JPQL で書くか、ネイティブクエリにするか~~ **解決済み。既定は JPQL、ネイティブは `WITH RECURSIVE` のように JPQL に無い機能を使うときだけ**（[4.3](#43-infrastructure)）| 完了 |
| 2 | ~~フロントエンドの API 型定義を手書きするか、OpenAPI から生成するか~~ **解決済み。手書き。** 画面が実際に使う項目だけを書く。ただし**手書きの型はコンパイラに守られていない**ので、実物のバックエンドへ当てる通しのシナリオを必ず置く（落とし穴 143）| 完了 |
| 3 | ~~監査証跡をアプリケーションで記録するか、DB トリガで記録するか~~ **解決済み。アプリケーションが書く。** トリガに残すのは `updated_at` と循環の拒否だけ（[6.5](#65-ログと監査)）| 完了 |

---

### 8.1 ルール自体を検査する

ルールが緑なのは「規約が守られている」からとは限らない。
**対象が 1 件も無い、条件が誤っていて何にも当たらない、でも緑になる。**
とくに実装が進んでいない段階では `allowEmptyShould(true)` を付けているので、
ルールは黙って何も検査しないでいられる（CLAUDE.md 落とし穴 30・31）。

`ArchRuleSelfTest` が、わざと違反したクラスに AR-01〜10 を当てて
**落ちることを確かめる。**

| 決定 | 理由 |
| --- | --- |
| 違反クラスを `jp.co.sample.probe`（`kintai` の外）に置く | Spring のコンポーネント走査と JPA のエンティティ走査は `@SpringBootApplication` のパッケージ配下を対象にする。内側に置くと、壊したクラスが本番の DI とマッピングに載る |
| 本番のルールは `DoNotIncludeTests` で除外し、**その除外自体も検査する** | 除外が効かなくなると、用意した違反が本番のルールを落とし、ルール全体を無効化する圧力になる |
| **条件ごとに違反クラスを 1 つずつ置く** | 1 つだけ踏んでも、禁止先を 5 個から 1 個に削ったルールが通ってしまう。AR-01 は禁止パッケージごと、AR-05 は「名前の規約」と「注釈」で分ける |
| **禁止先ごとに踏めているかを、変異で確かめる** | この規約は書いてあっても守られていなかった。禁止先を 1 つずつ削って自己検査が落ちるかを試したところ、**AR-06 の `presentation`・AR-03 の `presentation`・AR-10 の 3 コンテキストが素通りした**（下表）|
| **違反クラスを書けない禁止先は置かない** | AR-01 は `io.swagger..` を挙げていたが、springdoc は依存に入っておらず違反クラスを書けない。効いているか確かめようのない禁止先は、「守られている」という見かけだけを増やす。採用するときに違反クラスと一緒に戻す |
| 「落ちないこと」も検査する | AR-06 は自コンテキスト内の `infrastructure` 参照では落ちてはならない。ここを見落とすと、リポジトリの実装を書いた瞬間にルールが誤って落ちる |

#### 禁止先ごとの違反クラス

禁止先が複数ある規約は、**禁止先ごとに違反クラスを 1 つずつ**置く。

| 規約 | 禁止先 | 違反クラス |
| --- | --- | --- |
| AR-01 | `org.springframework..` | `UsesSpringFramework` |
| AR-01 | `jakarta.persistence..` | `UsesJakartaPersistence` |
| AR-01 | `com.fasterxml.jackson..` | `UsesJackson` |
| AR-01 | `org.hibernate..` | `UsesHibernate` |
| AR-02 | `..application..` | `ReachesIntoApplication` |
| AR-02 | `..infrastructure..` | `ReachesIntoInfrastructure` |
| AR-02 | `..presentation..` | `ReachesIntoPresentation` |
| AR-03 | `..infrastructure..` | `DependsOnAdapter` |
| AR-03 | `..presentation..` | `DependsOnController` |
| AR-06 | 他コンテキストの `infrastructure` | `AlphaNode` |
| AR-06 | 他コンテキストの `presentation` | `AlphaReachesPresentation` |
| AR-10 | `..kintai.employee..` | `SharedReachesIntoEmployee` |
| AR-10 | `..kintai.workrule..` | `SharedReachesIntoWorkRule` |
| AR-10 | `..kintai.attendance..` | `SharedReachesIntoAttendance` |
| AR-10 | `..kintai.approval..` | `SharedReachesIntoApproval` |
| AR-10 | `..kintai.leave..` | `SharedReachesIntoLeave` |
| AR-10 | `..kintai.payroll..` | `SharedReachesIntoPayroll` |

> **`SharedReachesIntoPayroll` は M3-a/C で置く。**
> 表に足しただけでは検査は 1 行も増えない。
> `ContextDependencyTest` の禁止先に `payroll` を足し、違反クラスを 1 つ置き、
> **禁止先から `payroll` を削ると自己検査が落ちること**を変異で確かめるところまでが 1 組である
> （落とし穴 84・86・87）。

**1 つのクラスで複数の禁止先を踏まない。**
片方を禁止先から外しても同じクラスが引っかかり続けるので、削られたことに気づけない。
...
これはどの業務コンテキストにも属さないので、この文書が正である。

| ID | 観点 | 期待 |
| --- | --- | --- |
| IT-SCH-01 | **設計書に挙げたテーブルがすべて存在する** | 欠けも余りも無い |
| IT-SCH-02 | **`btree_gist` 拡張が入っている** | `EXCLUDE USING gist` の前提（CLAUDE.md 落とし穴 7） |
| IT-SCH-03 | **`time_clock_events` がレンジパーティションになっている** | 親テーブルと子テーブルが揃う |
| IT-SCH-04 | **`updated_at` を持つ表すべてにトリガが張られている** | 更新時刻の自動更新 |

**Flyway が適用できること自体はテストの ID にしない。**
適用に失敗すればコンテキストの読み込みごと落ちるので、
すべての結合テストがその検査を兼ねている。ID を振ると、
「失敗した版が無い」という**恒真の検査**が 1 件増える（落とし穴 36）。

> **第 1 版はこの表と実装の ID が 1 つずつずれていた。**
> 表の 01（Flyway）に対応するテストが無く、実装の 01 は表の 02 を指していた。
> 実装にしかない観点（`updated_at` のトリガ）もあった。
> **同じ ID が設計書とコードで別の観点を指している**状態は、
> `collect-tests.py` の突き合わせでは検出できない（ID は両方に存在するため）。
> 表を書き換えたら、実装の `@DisplayName` と 1 行ずつ照らす。
