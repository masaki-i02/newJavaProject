...
> 出力する 1 行は `employee`・`attendance`・`approval` の値を束ねたものなので、
...
| ID | 観点 | 期待 |
| --- | --- | --- |
| IT-SCH-01 | **Flyway のマイグレーションが適用できる** | V1〜V6 に失敗した版が無い |
| IT-SCH-02 | **設計書に挙げたテーブルがすべて存在する** | 欠けが無い |
| IT-SCH-03 | **`btree_gist` 拡張が入っている** | `EXCLUDE USING gist` の前提（CLAUDE.md 落とし穴 7） |
| IT-SCH-04 | **`time_clock_events` がレンジパーティションになっている** | 親テーブルと子テーブルが揃う |
