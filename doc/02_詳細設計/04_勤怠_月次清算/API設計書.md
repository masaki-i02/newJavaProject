...
| ID | 観点 | 期待 |
| --- | --- | --- |
| IT-API-20 | 計算済みの月次清算を取得 | 値と版が返る |
| IT-API-21 | **固定時間制の応答** | 週の内訳が付く。フレックスには付けない |
| IT-API-22 | まだ計算していない月 | 404。**暗黙に計算しない** |
| IT-API-23 | 一般社員が他人の月次清算 | 403 |
| IT-API-24 | 人事が再計算 | 成功し、版が 1 つ上がる |
| IT-API-25 | 人事でない利用者が再計算 | 403 |
| IT-API-26 | **版が一致しない再計算** | 409。画面が見ていない結果を上書きしない |
| IT-API-27 | 締め済みの月を再計算 | 409 |
| IT-API-28 | **未計算の勤務日がある月を再計算** | 409。**どの日かを応答に載せる** |
| IT-API-29 | **走査範囲に入る前月の未計算日** | 検出する。清算期間だけを見ると取りこぼす |
| IT-API-30 | **締め済みの月へ打刻** | 409（403 ではない）。状態が変われば通る |
| IT-API-31 | 締めていない月へ打刻 | 成功する |
| IT-API-48 | **未来の時刻の打刻** | 422 `future-punch`。異常の受け皿（DEFAULT パーティション）に利用者が行を作れる状態にしない |
| IT-API-49 | **版を省いた再計算** | 400 `validation-failed` で `version` が名指しされる。`long` で受けると省略時に 0 が入り、**行が無い月では突き合わせが偶然通る**（落とし穴 57）|
| IT-API-36 | **限度時間を超えた社員が一覧に現れる** | 社員番号・氏名は返さない（`employee` が所有する概念）|
| IT-API-37 | **超えていない社員は現れない** | 全員を返して画面に絞らせると、判定が画面にも生まれる |
| IT-API-38 | 人事でない利用者の超過者一覧 | 403 |
...
| 2 | ~~月次清算の結果を CSV で出力する形式~~ **解決済み。**[給与連携 設計書](../07_給与連携/)が持つ（BR-18）。月次清算の API は変えない | 完了 |
