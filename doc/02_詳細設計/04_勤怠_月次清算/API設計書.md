...
### 過去月を計算し直したら、同じ年度の後続月も計算し直す

年度累計（`annualUsedBefore`）は**行に焼き付けてある**ので、
過去月が動いたのに後続月をそのままにすると、36 条 4 項の年 360 時間の判定が
**過少なまま残り続ける**。4 月が訂正で 50 時間から 80 時間に増えても、
5 月以降は 50 時間として数え続け、超過を取りこぼす。

連鎖の範囲は次のとおり。

| | 扱い | 理由 |
| --- | --- | --- |
| 同じ年度の後続月で**行がある**もの | 計算し直す | 年度累計が動くのはこの範囲だけ |
| 行が**無い**月 | 作らない | 行の有無は「その月に打刻があるか」を表す。ついでに作ると働いていない月の行ができる |
| **締め済み**の月 | 動かさない | 確定した値は動かせない（BR-10）。締めのあとに過去を直しても、その月の累計は当時のまま残る |
| **翌年度** | 触らない | 年度をまたいで累計は繰り越さない |
