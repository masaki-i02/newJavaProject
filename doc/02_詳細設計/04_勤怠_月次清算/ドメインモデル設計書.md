...
```java
// 通算の対象になるのは、法定休日の勤務日から持ち越された区間を受けた暦日だけ
Duration nonHoliday   = 暦日 D の区間のうち LEGAL_HOLIDAY が付いていないものの合計;
Duration alreadyBeyond = 暦日 D の区間のうち OVERTIME_BEYOND_STATUTORY が付いているものの合計;
Duration additional = max(ZERO, max(ZERO, nonHoliday - 法定労働時間) - alreadyBeyond);
```

`alreadyBeyond` を引くのが**二重計上を防ぐ要**である。3.1 の週 40 時間超と同じ形をとる。
...
| 週 40 時間超との関係 | 通算で法定外になった分を、その暦日が属する週の**法定内労働から引く** | 引かないと、法定外になった時間を週 40 時間の判定にも数えて二重計上する |
...
| UT-BR12-03 | 法定内残業のみ 50 時間 | 警告は立たない（時間外労働ではないので対象外） | BR-12 |
