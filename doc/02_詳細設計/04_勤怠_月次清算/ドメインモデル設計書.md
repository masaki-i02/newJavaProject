...
| `SettlementPeriod` | 本コンテキスト | 清算期間。`DateRange` の別名ではなく、月と期間の対応を持つ（2.0） |
...
**制度の判別に enum を新設しない。** 分岐は `workrule` の `sealed interface` に対する
網羅性検査つき `switch` で行い、永続化のときだけ判別値
（`working_time_system` 列の `'FIXED'` / `'FLEX'`）へ写す。
...
public record SettlementPeriod(YearMonth month, DateRange period) {
    ...
    public static SettlementPeriod of(YearMonth month, DateRange employmentPeriod) {
        DateRange calendarMonth =
                new DateRange(month.atDay(1), month.plusMonths(1).atDay(1));
        return new SettlementPeriod(month, calendarMonth.intersect(employmentPeriod));
    }

    public long days() { return period.days(); }
}
```
...
| 9/20 退職の最終月 | `[2026-09-01, 2026-09-21)` | **21** |
 ...
 */
public Duration statutoryTotalLimit(SettlementPeriod period, Duration statutoryWeekly) {
    long days = period.days();                               // 暦月ではなく清算期間の日数
    long minutes = days * statutoryWeekly.toMinutes() / 7;   // 整数除算 = 切り捨て
    return Duration.ofMinutes(minutes);
}
```

**`YearMonth` ではなく `SettlementPeriod` を受ける。**
...
| 取得する日次の範囲 | **対象月の初日を含む週の起算日から、翌月初日を含む週の起算日まで** | 月初の週は前月の日を含む。前月の日が欠けると週の法定内労働が過少になり、週 40 時間超を取りこぼす |

```java
/** 週次判定に必要な日次の範囲。対象月の範囲より広い。 */
public DateRange weeklyScanRange(SettlementPeriod period) {
    LocalDate from = period.period().from()
            .with(TemporalAdjusters.previousOrSame(DayOfWeek.SUNDAY));
    LocalDate toExclusive = period.period().toExclusive()
...
public record MonthlySettlement(
        ...
        WorkingTimeSystem workingTimeSystem,   // workrule の sealed interface
    ...
    public MonthlySettlement {
        ...
        boolean fixed = workingTimeSystem instanceof FixedTimeSystem;
        if (fixed && !overtimeTime.equals(dailyOvertimeTime.plus(weeklyOvertimeTime))) {
            throw new IllegalStateException("時間外労働の内訳が一致しません");
        }
        if (!fixed && !(dailyOvertimeTime.isZero() && weeklyOvertimeTime.isZero())) {
            throw new IllegalStateException("フレックスに日次・週次の時間外を計上しています");
        }
...
| UT-BR05-12 | **9/20 退職の最終月の総枠** | 21 日ぶんの 7,200 分 | BR-05 |
...
| UT-BR05-19 | **月中に就業規則が改定された月** | 日ごとに版を引き、改定日の前後で切り替わる | BR-05 |
