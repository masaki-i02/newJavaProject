...
> `CHECK` 制約は他テーブルを参照できないので、値を式に直書きしている。
> **これが成り立つのは、法定労働時間が入力ではないからである**（未決事項 #1）。
> 1 日 8 時間・1 週 40 時間は法が決める定数であり（労基法 32 条）、
> 会社が設定するのは<strong>所定</strong>労働時間のほうである。
> `POST /api/work-rules` は法定労働時間を受け取らない。
>
> **受け取っていた頃は、40 時間未満の規則を作れた。**
> ドメインは「40 時間以下」しか課さないので登録は通り、
> その社員の月次清算だけがこの制約に弾かれて
> <strong>永久に保存できなかった</strong>（理由の載らない 500・IT-API-46）。
...
**#3 は欠番。** 番号を詰め直さない。
本文（3.2）が #1 を名指しで参照しており、詰めると参照先がずれる。

| # | 内容 | 判断の時期 |
| --- | --- | --- |
| 1 | ~~週の法定労働時間を設定可能にするか~~ **解決済み。設定項目にしない。** 法定労働時間は法が決める定数（労基法 32 条）であって会社の設定ではない。会社が定めるのは所定労働時間のほうである。**API は法定労働時間を受け取らない**（IT-WR-44）。受け取っていた頃は、40 時間未満の規則を作ると**その社員の月次清算が永久に保存できなかった**（IT-API-46）。制約の 2400 直書きは、この決定によって初めて正しい主張になる | 完了 |
| 2 | ~~週の起算曜日を設定可能にするか~~ **解決済み。設定項目にしない。日曜固定。** 保つのは `WeeklyOvertimeRule.WEEK_START` と `weekly_overtimes_start_dow_check` の 2 か所（[04 ドメインモデル設計書 7 章](ドメインモデル設計書.md) の #1 と同じ決定） | 完了 |
| 3 | **欠番**（詰めない） | — |
| 4 | ~~36 協定の閾値を `work_rules` 側の設定項目にするか~~ **解決済み。置かない。** 清算時点の閾値を `monthly_settlements` の列へ写して保存する（`monthly_agreement_limit_minutes` / `annual_agreement_limit_minutes`）。列の値で `CHECK` するので、**閾値を変えても過去の行の判定は当時の閾値のまま**である。特別条項は対象外 | 完了 |
