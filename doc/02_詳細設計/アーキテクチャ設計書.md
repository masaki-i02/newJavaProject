...
flowchart LR
    U["利用者<br/>ブラウザ"]
    F["フロントエンド<br/>React + TypeScript"]
    B["バックエンド<br/>Spring Boot 4.1 / Java 21"]
    D[("PostgreSQL 17")]
...
```mermaid
flowchart TB
    P["presentation<br/>REST コントローラ / DTO"]
    A["application<br/>ユースケース / トランザクション境界"]
    D["domain<br/>業務ルール / ポート定義"]
    I["infrastructure<br/>JPA エンティティ / ポート実装"]
...
│   └── presentation/
├── attendance/
│   ├── domain/                      TimeClockEvent, WorkSlice, PremiumType,
│   │                                DailyAttendance, MonthlySettlement,
│   │                                計算エンジンと規則
...
public interface DailyAttendanceRepository {
    void save(EmployeeId employeeId, DailyAttendance attendance);
    List<DailyAttendance> findByPeriod(EmployeeId employeeId, LocalDate from, LocalDate to);
}
...
| 業務判断 | 「対象月時点の所属部署の上長か」 | `approval/domain` の `ApproverResolver` |
...
| DB | PostgreSQL 17 | `EXCLUDE` 制約・範囲型・パーティション・配列型を使う。これらが本システムの設計の土台 |
