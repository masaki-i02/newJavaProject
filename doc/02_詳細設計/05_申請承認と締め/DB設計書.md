...
`monthly_attendances` の行は提出時に初めて作られるので、
「行が無い月」と「作られたばかりの月」の 2 つの状態がある。
どちらの版も 0 だと**この 2 つが区別できない。**

区別できないと次が起きる。承認者が提出前の画面（版 0）を開いたまま、
別の経路で提出が行われる。行が作られても版は 0 のままなので、
承認者が握っている**古い版がそのまま一致し、楽観ロックが素通りする。**

行の作成時に 1 を入れれば、0 は「行が無い」ことだけを指す。
`monthly_settlements` も同じ約束で、こちらは行の入れ替えのたびに版を 1 つ進める。
