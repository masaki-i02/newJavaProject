...
| 遷移 | 遷移元 → 遷移先 | 実行者 | 事前条件 |
| --- | --- | --- | --- |
| `Submit` | 下書き → 提出済 | 本人、または **`HR`（本人が在籍していない場合）** | 対象月の末日が到来していること／**打刻があるのに日次勤怠が無い勤務日が残っていないこと**（2.3） |
...
| 事前条件 | 判定する場所 |
| --- | --- |
| 対象月の末日が到来していること | `approval`（`MonthlyAttendanceService`） |
| **打刻があるのに日次勤怠が無い勤務日が残っていないこと** | `attendance`（`MonthlySettlementService.requireCalculable`） |

```java
// approval --- 手順を組むだけ。日次が出そろっているかの判定は attendance に委ねる
...
第 2 版は「在籍中の所定労働日すべてに日次勤怠の行があること」を求めていた。
日次勤怠の行は**打刻がそろった日にしか作られない**ので、
これは「全所定労働日に出勤していること」と同じ条件である。

**1 日でも欠勤すると、その月を永久に提出できなかった。**
欠勤の日を確定させる手段はどこにも無い。
打刻を足せば、働いていない日の労働時間が一次証拠として残る（BR-09 の前提が壊れる）。
年次有給休暇を取得した日も同じで、[06](../06_年次有給休暇/ドメインモデル設計書.md) が
日次勤怠に行を作らない以上、必ず未確定に数えられた
（CLAUDE.md 落とし穴 19・70 と同型）。

確かめるべきは **「打刻があるのに日次勤怠が無い日」** である。
これは「未退勤のまま放置された日」と「就業規則が無くて計算できなかった日」を指す。
打刻が 1 件も無い日は欠勤・年休・休業のいずれかであり、確定させる対象ではない。

#### 判定を `approval` に写さない

この判定は月次清算がすでに持っている（[月次清算 API設計書 3](../04_勤怠_月次清算/API設計書.md)）。
写すと、丸めや走査範囲を直したときに片方だけが古くなる（落とし穴 67）。
`MonthlySettlementService.requireCalculable` を公開し、提出はそれを呼ぶ。

> **`shared.domain.CalculatedWorkDates` は廃止した。**
> 「計算が確定している勤務日」を返すポートだったが、
> 呼ぶ側が「所定労働日すべてがその集合に入っているか」を判定していたため、
> **欠勤の日を確定させる手段が無い**という上記の欠陥をポートの形が誘発していた。
> 走査範囲（週次判定のため前月へ広げる）も呼ぶ側は知りようがない。
...
警告は承認者が見るべき情報だが、**承認待ち一覧の項目にはしない。**
同じ事実は各日の `breakRequirementSatisfied` が持っており、
2 か所に置くと BR-08 の判定を直したときに片方だけが古くなる（落とし穴 64）。
承認者は詳細（SC-06）で日次を引き、不足した日を強調して見る
（[API設計書 2.6](API設計書.md)）。
...
| --- | --- |
| 1 人以上 | そのいずれかが承認する |
| **0 人**（`HR` が対象社員 1 人だけ） | **その月は決裁できない。** 人事担当を 2 名以上置くことで避ける |

**`ADMIN` へはエスカレーションしない。**
`ADMIN` は社員マスタとロールの管理者であって、**承認の権限を持たない**
（`Approver.isApprovedBy` が見るのは `HR` だけである）。
`ADMIN` に無条件の承認権を与えると、BR-11 の 1〜4 をすべて迂回できてしまう。

> 版 1.3 まで「`ADMIN` へエスカレーションする」と書いていたが、
> **そのような経路は実装されていない。**
> `ADMIN` は `HR` を付与できる立場なので、
> 詰まった場合の出口は「人事担当をもう 1 人立てる」ことである。
> **書いてあるだけの逃げ道は、無いより悪い**（落とし穴 87）。
>
> **7 章の観点表（`UT-BR11-13`）だけが版 1.3 の記述のまま残っていた。**
> 本文が「しない」、表が「する」と、同じ文書の中で割れていた
> （落とし穴 182 と同型）。本文を直したときは、
> **その規則を参照している観点表とテストケース一覧も同じコミットで直す。**

**人事担当を 2 名以上置くことは運用上の前提である。**
1 名しかいない状態でその 1 名の月が `HUMAN_RESOURCES` に落ちると、
承認も差戻しもできず<strong>その月を永久に締められない</strong>（落とし穴 26）。
...
public record CorrectionRequest(
        ...
        Optional<EmployeeId> decidedBy,       // 取下げでは本人が入る
        Optional<LocalDateTime> decidedAt,
        Optional<String> decisionComment) {

    public CorrectionRequest {
        // 理由と内容は必須（省略）
        // 状態と決裁の記録の整合を、DB の correction_requests_state_check と同じ形で守る
        if (status.isPending() != decidedBy.isEmpty()) { ... }
        if (decidedAt.isPresent() && decidedAt.get().isBefore(requestedAt)) { ... }
        if (status == REJECTED && 理由が空) { ... }
        if (status != CANCELED && decidedBy が本人) { ... }   // 自己決裁の禁止
        if (status == CANCELED && decidedBy が本人でない) { ... } // 取下げは本人だけ
    }
}
...
**`Revoke` は `workDate` を持たない。**
当初は「パーティションを絞るため」に持たせる設計だったが、
申請そのものが `workDate` を持つので<strong>同じ値が 2 か所に現れる。</strong>
食い違ったときにどちらが正しいか決められないため、申請の側にだけ置く。

**`Add` は打刻種別と時刻ではなく `TimeClockEvent` を持つ。**
種別と時刻を別々に持つと「種別に合わない時刻」を組み立てられるが、
`TimeClockEvent` は `sealed interface` なので、
<strong>作れる組み合わせが型の側で閉じている。</strong>

**打刻の識別子は `attendance` が持つ。**
打刻そのもの（`TimeClockEvent`）は識別子を持たない。
労働時間の計算に要るのは種別と時刻だけで、識別子を持たせると
値としての等価性が壊れるためである。識別子が要るのは訂正だけなので、
`RecordedTimeClockEvent`（識別子つきの打刻）を `attendance.domain` に置いて外から添える。
...
public interface MonthClosureQuery {
    ...
    // ★ 2.1 と同じ理由で 2 つに分ける。1 つにまとめると、提出済みの月で true を返し、
    //   本人が提出後に直接打刻できてしまう
...
| UT-BR10-08 | 締め済みからの遷移 | **実行時に拒否する**（`InvalidTransitionException`／409） | BR-10 |
...
| UT-BR10-17 | **休憩不足の警告が立ったまま提出** | 提出できる。承認者は詳細で日次の `breakRequirementSatisfied` から見る | BR-08 |
| UT-BR10-18 | **月中入社の初月の提出** | 勤務日は入社日から数える。初月が締められる | BR-10 |
| UT-BR10-19 | **承認済が持つ記録** | 提出者・提出日時・承認者・承認日時をすべて持つ | BR-10 |
| UT-BR10-20 | **本人が自分の勤怠を承認**（集約に直接） | `SelfApprovalException`／403 | BR-11 の 4 |
| UT-BR10-21 | **代理提出された月を本人が承認** | 同じく拒否。**比べる相手は提出者ではなく対象社員** | BR-11 の 4 |
| UT-BR10-30 | **本人が自分の勤怠を差し戻す**（集約に直接） | `SelfApprovalException`／403。承認だけを塞いでも差戻しが空いていた | BR-11 の 4 |
| UT-BR10-22 | 下書きを承認 | `InvalidTransitionException` | BR-10 |
| UT-BR10-23 | 提出済を締める | 拒否。承認を経る | BR-10 |
| UT-BR10-24 | 二重提出（集約に直接） | 拒否 | BR-10 |
| UT-BR10-25 | **訂正による差戻しを提出済以外から** | 拒否。アプリケーション層は何もしないので集約で検査する | BR-09 / BR-10 |
| UT-BR10-26 | 下書きが受け付ける操作 | 打刻・訂正申請のどちらも受け付ける | BR-10 |
| UT-BR10-27 | **承認が提出より前の承認済** | 生成時に例外 | BR-10 |
| UT-BR10-28 | **承認が提出より前の締め済** | 生成時に例外。永続化アダプタが行から直接組み立てる経路がある | BR-10 |
| UT-BR10-29 | **締めが承認より前の締め済** | 生成時に例外 | BR-10 |
| UT-BR11-13 | **対象社員本人が唯一の `HR` で人事へ到達** | **その月は決裁できない**（自己決裁の禁止が先に働く）。`ADMIN` へはエスカレーションしない（3.2） | BR-11 |
| UT-BR11-16 | **個人承認（`INDIVIDUAL`）の月** | その承認者本人だけが承認でき、他の社員は承認できない | BR-11 の 3 |
| UT-BR11-17 | **人事承認（`HUMAN_RESOURCES`）の月** | `HR` を持つ社員なら誰でも承認でき、持たない社員は承認できない | BR-11 の 5 |
| UT-BR11-18 | **承認者が決まらない月（`NONE`）** | 誰も承認できない。人事でも承認できない | BR-11 |
| UT-BR11-14 | **自部署は長が未設定、親は本人、その親の長が退職済み** | 経路に 3 つの理由が順に記録される | BR-11 |
| UT-BR11-15 | **月中に所属が始まった社員の基準日** | 所属開始日が基準日になる | BR-11 |
...
| UT-BR09-10 | **訂正内容が空の申請** | 生成時に例外 | BR-09 |
| UT-BR09-11 | **決裁が申請より前の申請** | 生成時に例外。永続化アダプタが行から直接組み立てる経路がある | BR-09 |
| UT-BR09-12 | **申請済なのに決裁者を持つ申請** | 生成時に例外 | BR-09 |
| UT-BR09-13 | **本人が自分の訂正を承認** | `SelfDecisionException`／403 | BR-09 |
| UT-BR09-14 | **本人が自分の訂正を却下** | 同じく拒否。承認だけを塞いでも意味がない | BR-09 |
| UT-BR09-15 | 却下の理由が空白のみ | 生成時に例外 | BR-09 |
| UT-BR09-16 | **他人が申請を取り下げる** | `NotTheRequesterException`／403。取下げは本人の意思表示 | BR-09 |
| UT-BR09-17 | **決着済みの申請を再び決裁** | `AlreadyDecidedException`／409 | BR-09 |
| UT-BR09-18 | **取消と追加で打刻を差し替える** | 退勤だけが差し替わる。「変更」を用意しない | BR-09 |
| UT-BR09-19 | **実在しない取消対象** | 申請の時点で検出し、その ID を返す。外部キー違反にしない | BR-09 |
| UT-AUD-01 | すべての状態遷移 | `ApprovalEvent` が記録される | 要件 7 章 |

**UT-BR10-08 は実行時の検査になった。**

当初は「締め済みからの遷移は型として存在しないので、書こうとするとコンパイルエラーになる」
と書いていたが、実装ではそうならなかった。
遷移メソッドを状態（`MonthlyAttendanceStatus` の各 `record`）ではなく
**集約（`MonthlyAttendance`）に置いた**ためである。
自己承認の禁止（BR-11 の 4）には**対象社員の ID** が要り、
それを持っているのは状態ではなく集約だからで、
その結果 `closed.reject()` はコンパイルが通る。

確定値が動かないことは `requireStatus` が実行時に保証し、UT-BR10-08 がそれを検査する。
状態ごとに遷移メソッドを分ければコンパイル時に倒せるが、
自己承認の判定を別の型へ逃がすことになるので採らない。
...
| # | 内容 | 判断の時期 |
| --- | --- | --- |
| 1 | ~~提出・承認・差戻しの通知（メール等）を送るか~~ **解決済み。送らない**（BR-13 でメール基盤を持たないと決めた）。代わりに**引く経路**を用意する ─ 承認待ち一覧（`GET /api/monthly-attendances/pending-approval`）と `availableActions` / `canSubmit` / `canApprove` | 完了 |
| 2 | ~~承認者が長期不在の場合の代理承認~~ **解決済み。設けない。** `Approver` が `INDIVIDUAL` のときは本人一致でしか承認できず、人事でも代われない。無条件の承認権を与えると BR-11 の 1〜4 をすべて迂回できる。不在時は `ADMIN` が**部署長を交代させる**（3.9）。ただし交代は基準日で効くので、過去の月の承認者は変わらない（締め済みの月へは遡れない・IT-EMP-74）| 完了 |
| 3 | ~~締め済みの月をやむを得ず訂正する手段~~ **解決済み。作らない。締め済みは不可逆とする。** `MonthlyAttendance` に `Closed` を入口とする遷移は 1 つも無く、打刻・訂正・年休・就業規則・カレンダー・異動・退職・部署長のすべてが締めを見る。誤りは**締める前に承認を取り消して**直す | 完了 |
| 4 | ~~`HR` が 1 人だけの場合に `ADMIN` へエスカレーションするか~~ **解決済み。しない**（3.2）。`ADMIN` は承認の権限を持たない。**人事担当を 2 名以上置くことが運用上の前提**である | 完了 |
| 5 | ~~代理提出できるのを `HR` に限るか~~ **解決済み。`HR` に限る。しかも対象は退職者だけ。** 在籍している社員は本人が提出する（IT-APV-40）。`ADMIN` には認めない。**理由が必須**で、無いと 422（IT-APV-99）| 完了 |
