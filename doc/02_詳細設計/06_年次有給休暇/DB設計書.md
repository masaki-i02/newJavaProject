...
IT-LV-28 は、外部キーを満たす実在の就業規則系列を使う。
満たさないと `work_rule_series_id_fkey` に先に掛かり、狙った検証にならない。

### 7.1 永続化アダプタの観点

制約とは別に、**ドメインの record と行を往復して同じ値になる**ことを確かめる。

| ID | 観点 | 期待 | 参照 |
| --- | --- | --- | --- |
| IT-LV-99 | 付与を保存して読み戻す | 同じ値になる | BR-14 |
| IT-LV-100 | **不付与の付与も読み戻せる** | `Withheld` として復元される | BR-14 |
| IT-LV-101 | **出勤扱いの日数と理由が読み戻せる** | 判定の根拠が残る | BR-14 |
| IT-LV-102 | **新しい付与の版は 1 から始まる** | 既定値に任せない | BR-14 |
| IT-LV-103 | 古い版で再判定する | `OptimisticLockingFailureException` | BR-14 |
| IT-LV-104 | **失効した付与も読み出せる** | 絞るのはドメイン。SQL に写すと 1 日ずれる（落とし穴 91） | BR-15 |
| IT-LV-105 | 申請を保存して読み戻す | 同じ値になる | BR-16 |
| IT-LV-106 | **承認すると配分先が読み戻せる** | 複合外部キーを満たす | BR-15 |
| IT-LV-107 | **取り消しても承認者が残り、配分は外れる** | 残日数が戻り、承認の履歴も追える | BR-16 |
| IT-LV-108 | 古い版で更新する | `OptimisticLockingFailureException` | BR-16 |
| IT-LV-109 | **新規申請の証跡は遷移元が `NONE`** | 自分自身への遷移にしない | 要件 7 |
| IT-LV-110 | **`PaidLeaveDays` は承認済みの取得日だけを返す** | 未処理は含めない | BR-05 / BR-16 |
| IT-LV-111 | **期間の外にある取得日は返さない** | 清算期間で絞る。月中入社・月中退職で効く | BR-05 |
...
| # | 内容 | 判断の時期 |
| --- | --- | --- |
| 1 | ~~付与の再判定を証跡テーブルに残すか~~ **解決済み。残さない**（ドメインモデル設計書 7 章 #2）| 完了 |
| 2 | ~~状態の一致を制約トリガで守るか~~ **解決済み。トリガは置かない。** 05 の `approval_events` と同じ形で、状態と証跡を同じトランザクションの単一経路で書く。DB 側は遷移の組だけを見る | 完了 |
| 3 | 休業（労災・産育休）の記録表。BR-14 の出勤扱いに要る | **行き先を書かない**（ドメインモデル設計書 7 章 #1 と同じ項目）|
