...
| `GET` | `/api/paid-leave-requests/pending-approval` | 承認待ちの一覧 | 本人 / 承認者 / `HR` |
| `POST` | `/api/paid-leave-grants` | 付与の実行（基準日まで） | `HR` |
| `POST` | `/api/employees/{id}/paid-leave-grants/{grantedOn}/reassessment` | 出勤率の再判定 | `HR` |
| `GET` | `/api/paid-leave/obligations` | 年 5 日が未達の社員（BR-17） | 本人 / 承認者 / `HR` |

**ロールで一律に拒まない。** 一覧を返す 2 つは閲覧範囲（`EmployeeVisibility`）で絞るので、
一般社員が呼ぶと**自分のぶんだけ**が返る。ロールで拒むと、
自分があと何日年休を取る必要があるかを本人が確かめられなくなる。
...
**配列を裸で返さずオブジェクトで包む。**
**解決した `asOf` を必ず載せる。** 既定値は `application` 層が `Clock` から決めるので、
返さないと利用者は「いつ時点の一覧なのか」を確かめられない。

**進行中の義務期間だけを対象にする。** 付与日から 1 年を過ぎた義務は
もう是正できないので、「あと何日取らせるか」の材料にならない。
...
**過去の取得日も受け付ける。** 打ち忘れと同じで、事後に申請せざるを得ない場面がある。
締め済み・承認済みの月は別のエラーで拒むので、遡って勝手に確定した月は動かせない。
なお `availableDays` の窓は「当日から 1 年」で非対称だが、
これは<strong>これから何日申請できるか</strong>を示す値であり、過去日の受理とは別の問いである。
...
**識別子ではなく付与日を返す。** 識別子だけでは、画面がもう一度残日数を引かないと
いつ失効するのかが分からない。

**`grantedOn` は決裁の応答にだけ載せる。**
一覧で行ごとに引くと、申請数ぶんの問い合わせが増える。
どの付与から消化したかを知りたいのは、いま決裁した 1 件だけである
（一覧に版を載せないのと同じ理由）。

**3 の結果を応答に含める。** 提出済みだった月が下書きに戻ることを伝えないと、
再提出が忘れられる（訂正の承認と同じ）。

**月次清算は含めない。** 年休の応答に他コンテキストの集計を埋め込むと、
清算の項目が増えるたびにこの応答も変わる。
必要なら {@code GET .../monthly-settlements/{month}} を引く
（[設計規約チェックリスト 3](../00_共通/設計規約チェックリスト.md)）。

**版は読み直して返す。** 更新は SQL 側で 1 つ進めるので、
遷移させた集約は古い版を持ったままである。返さないと画面は必ず 1 回 `409` を踏む。
...
| **承認は開けない** | 在籍していない日の年休を通しても、残日数が減るだけで社員には何も残らない。人事は承認者ではないので `403 not-approver` が受ける（認可の検査が業務の検査より先にあるため、`leave-date-not-in-service` までは進まない）|
...
| `422 deemed-attendance-rejected` | **出勤日 + 出勤扱いが全労働日を超える**。負の日数と理由の欠落は `presentation` が 400 で先に弾くので、ここはすり抜けた場合の防波堤である。`IllegalArgumentException` のまま素通しすると理由の載らない 500 になり、何日なら通るのかが人事に伝わらない |
...
| IT-LV-129 | **不付与の年も要素として返す** | `granted: false`。`days` / `usedDays` / `remainingDays` は項目ごと省き、`attendanceRate.deemedReason` の `null` は残す（落とし穴 76）| BR-14 |
| IT-LV-32 | **未処理の申請が `availableDays` から引かれる** | 申請の前後で `availableDays` が 1 減る。**`remainingDays` > `availableDays` は成り立たない**（未到来の付与を数えるので保有日数より大きくなりうる・IT-LV-98）| BR-16 |
| IT-LV-33 | 他人の残日数を配下でない社員が見る | `403 forbidden` | 要件 4.1 |
| IT-LV-34 | 承認者が配下の社員の残日数を見る | `200 OK` | 要件 4.1 |
| IT-LV-130 | **決裁した申請を読み戻せる** | 永続化された状態と版が `GET` から読める | BR-16 |
| IT-LV-131 | **配下でない社員の申請は 1 件でも読めない** | `403 forbidden` | 要件 4.1 |
| IT-LV-132 | その社員の申請の一覧を承認者が見る | 配下の申請が返る | 要件 4.1 |
| IT-LV-133 | **配下でない社員の申請は一覧に出ない** | 空になる | 要件 4.1 |
| IT-LV-134 | **承認待ちの一覧は配下だけを返し、版を載せない** | 行ごとに版を引くと社員数ぶんの問い合わせが増える（1.1）| 要件 4.1 |
| IT-LV-135 | 決裁した申請は承認待ちの一覧から消える | 未処理だけが残る | BR-16 |
...
| IT-LV-125 | **承認者でない社員には、対象月の状態より先に `not-approver` を返す** | 締め済みの月で試す。順序が逆だと `month-already-closed` が返り、対象社員の月の状態が漏れる | 要件 4.1 |
...
| IT-LV-126 | **他人の取下げには、対象月の状態より先に `not-the-requester` を返す** | 締め済みの月の承認済み年休で試す | 要件 4.1 |
...
| IT-LV-57 | **本人が取り下げると所定総が戻る** | 不足時間が 8 時間になる。**取消（`revocation`）ではなく取下げ（`cancellation`）で見る**。両方を取消で書くと、取下げの副作用の分岐を 1 件も通らない | BR-05 / BR-16 |
...
| IT-LV-127 | **付与済みの再判定は、日数の検証より先に 409 で拒否する** | 過大な日数を送っても 422 にならない。順序が逆だと「日数を直せば通る」と読める（落とし穴 17・25）| BR-14 |
| IT-LV-65 | 年 5 日の未達一覧 | 不足のある社員だけが返る | BR-17 |
| IT-LV-78 | **未達一覧に配下でない社員が現れない** | 閲覧範囲で絞られる | 要件 4.1 |
| IT-LV-123 | **期限の過ぎた義務は未達一覧に現れない** | 進行中の義務期間だけを対象にする。過ぎたものは是正できないので材料にならない | BR-17 |
| IT-LV-124 | **5 日取得済みの社員は `onlyShortfall` で絞られる** | `onlyShortfall=false` なら充足も返る | BR-17 |
| IT-LV-136 | **一般社員には自分の義務だけが返る** | ロールで一律に拒まない。自分があと何日必要かは本人が確かめられなければならない | BR-17 / 要件 4.1 |
| IT-LV-137 | **人事の社員が自分の年休を取り消す** | 403 `self-approval`。取消も決裁であり、理由つきで 5 年残る証跡になる。申請・承認・却下・取下げは本人を弾いており、取消だけが非対称だった（落とし穴 171）| BR-11 の 4 / BR-16 |
