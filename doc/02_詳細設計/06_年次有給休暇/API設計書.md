...
| `availableDays` | `remainingDays` から**未処理の申請**を差し引いた、いま申請できる日数 |
...
| `403 forbidden` | **自分の申請を自分で承認した**（自己承認の禁止・BR-11） |
...
| `409 leave-not-cancelable` | **取得日の当日以降** |
| `409 month-already-closed` | 対象月が締め済み |
| `409 invalid-transition` | 既に却下済み・取下げ済み |

**当日以降は取り消せない**（BR-16）。その日の勤怠は実績として確定しており、
直すなら訂正申請（BR-09）の領分になる。
...
出勤率を現在の実績で判定し直す。訂正申請（BR-09）で欠勤が出勤に直った場合に使う。
...
```json
{
  ...
  "granted": true,
  "days": 11,
  "attendanceRate": { "totalWorkingDays": 245, "attendedDays": 198 }
}
```
...
| IT-LV-45 | **自分の申請を自分で承認** | `403 forbidden` | BR-11 |
| IT-LV-46 | 承認者でない社員による承認 | `403 not-approver` | BR-11 |
| IT-LV-47 | **古い `version` での承認** | `409 optimistic-lock-failure` | BR-10 |
...
| IT-LV-51 | **取得日の前日に取り下げる** | `200 OK`・残日数が戻る | BR-16 |
| IT-LV-52 | **取得日の当日に取り下げる** | `409 leave-not-cancelable` | BR-16 |
