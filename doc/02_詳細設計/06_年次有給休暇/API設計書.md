...
| `availableDays` | **申請できる期間（当日から 1 年）のどこかで有効な付与の残から、未処理の申請を仮に配分した残り。** 件数の引き算ではない。基準日で絞ると未到来の付与が落ち、表示より多く申請できてしまう（[ドメインモデル設計書 3.4](ドメインモデル設計書.md)） |
...
**本人かどうかを最初に検査する。** 集約の検査（落とし穴 58 のために残す）に任せると、
その前に並ぶ 5 つの検査が**対象社員に対して**実行される。
他人の社員 ID を投げるだけで、締め状態・在籍・既存の申請・残日数が
エラーの型と `detail` から読み取れてしまう（要件 4.1）。
...
**承認の時点で、所定労働日・在籍ももう一度確かめる。**
申請から承認までの間に、人事がその日を所定休日へ変えたり、
その社員の退職が登録されたりしうる。月次清算の側は
「清算期間内 ∩ `WORKDAY`」で絞るので所定総は守られるが、
`leave` の側では**年休 1 日が消費されたまま**になる。
休日や在籍期間の外の日に年休を消費しても、社員には何も残らない。
...
未来日の年休を申請したあとにその社員の退職が登録されると、
取得日の月には所属が無いので `ApproverPolicy` は承認者を返さない
（`Approver` の `NONE`）。本人は退職後ログインできないので取り下げもできず、
**その申請はどの状態にも遷移できないまま、部分一意インデックスがその日を占有し続ける**
（落とし穴 26・93）。

| 決定 | 理由 |
| --- | --- |
| **承認者が導出できない申請は `HR` が却下できる** | 遷移先の無い行を残さない。理由が必須なので、何が起きたかは証跡に残る |
| **承認は開けない** | 在籍していない日の年休を通しても、残日数が減るだけで社員には何も残らない。`leave-date-not-in-service` が受ける |
| **`ApproverPolicy` は変えない** | 月次勤怠では「所属が無い月」に行が作られないので `NONE` は起きない。共通のポリシーを年休の都合で曲げると、BR-11 の意味が 2 つになる |
...
```json
{
  ...
  "failed": [
    { "employeeId": "...", "reason": "DataIntegrityViolationException" }
  ]
}
```
...
| **1 人の計算が失敗しても残りの社員を続ける**（`failed` に入れる） | 例外でループを抜けると、その日は以降の社員が一件も処理されず、結果も返らないので誰も気づけない。付与は冪等なので、原因を取り除けば翌日の実行で追いつく |
| `failed` の `reason` は**例外の種別だけ**。メッセージもスタックトレースも載せない | 制約名やテーブル名は内部の構造であり、外に出さない |
| 日次バッチは**毎日 02:30 に実行する**（`kintai.paid-leave.grant-scheduler.cron`） | 付与日は 0 時に到来するので、その日のうちに行が作られる |
| バッチは**設定で止められる**（`kintai.paid-leave.grant-scheduler.enabled`）。テストでは無効にする | 固定した時計とは無関係に実時刻で走り、テストの前提を黙って書き換える |
...
| `400 validation-failed` | `deemedAttendedDays` が 1 以上なのに `deemedReason` が空（`presentation` の入力検査。すり抜けた場合は下の 422 が受ける）|
| `403 forbidden` | 実行者が `HR` でない |
| `404 resource-not-found` | その日の付与が無い |
| `409 grant-already-granted` | **付与済みの付与を再判定しようとした** |
| `422 deemed-attendance-rejected` | **出勤日 + 出勤扱いが全労働日を超える／日数が負／1 以上なのに理由が空**。`IllegalArgumentException` のまま素通しすると理由の載らない 500 になり、何日なら通るのかが人事に伝わらない |
...
| IT-LV-121 | **食い違いが無ければ `warnings` の項目そのものを返さない** | 空配列にしない。項目を残すと「警告が無い」と「この操作では警告を返さない」を区別できない（落とし穴 76）| BR-16 |
...
| # | 内容 | 判断の時期 |
| --- | --- | --- |
| 1 | ~~日次バッチの実行時刻~~ **解決済み。毎日 02:30**（`kintai.paid-leave.grant-scheduler.cron` で変更できる）。**実行結果の通知先は未定。** いまは結果を戻り値で返すだけで、人事が `POST /api/paid-leave-grants` で追いつかせる経路がある | 実行時刻は完了 |
| 2 | 退職時に未消化の年休をどう扱うか（買上げは法定外。要件に無い） | 要件の改訂が要る |
| 4 | **出勤率の算定が会社カレンダーを 1 日ずつ引く**（算定期間 1 年で約 365 クエリ／社員）。100 名の付与日が重なる日は数万クエリになる。`CompanyCalendarRepository.findByPeriod` があるので、`CompanyCalendar` に「期間ぶんを一度に引く」経路を足せば 1 クエリで済む。`MonthlySettlementCalculator.workdayCountIn` も同じ形なので、直すなら共通で | 100 名では実害が出ていないので、実測してから |
