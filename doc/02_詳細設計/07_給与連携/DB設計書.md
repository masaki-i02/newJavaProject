...
| # | 内容 | 判断の時期 |
| --- | --- | --- |
| 1 | ~~出力した CSV のハッシュを記録に残すか~~ **解決済み。残さない。** 記録から同じ CSV を何度でも作り直せるようにしてある ─ ①対象社員を行として固定（落とし穴 122）、②分母を値ごと保存、③締め済みの月の値は動かない。ハッシュが答えるのは「外部に渡ったファイルが改竄されていないか」で、本文を保存しない以上その用途は本システムの責務外である | 完了 |
| 2 | ~~社員番号の再利用そのものを禁じるか~~ **解決済み。禁じない。** 部分一意インデックス（`WHERE retired_on IS NULL`）のままとし、衝突した月は**両名を CSV から除外**して人事に割り当て直させる（`DUPLICATE_EMPLOYEE_NUMBER`・落とし穴 121）。ただし**在籍中の再利用は塞いだ** ─ 退職日が未来の社員の番号は渡せない（IT-EMP-73）。認証も番号で 1 件に絞らない（IT-AUTH-23）| 完了 |
