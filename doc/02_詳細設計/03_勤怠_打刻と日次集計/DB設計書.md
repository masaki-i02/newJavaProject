...
**運用者による直接操作は DB 権限では抑止しない**（未決事項 #2）。
アプリケーションとマイグレーションを **1 つのロールで動かす**と決めているので
（[デプロイ手順書](../../08_運用/デプロイ手順書.md) 3 章）、
同じロールに `REVOKE` しても意味がない。
**書いても効かない守りを一覧に載せない**（落とし穴 87）。
...
| 打刻テーブルへの UPDATE / DELETE | **アプリケーション（ポートの形）。** DB 権限では守らない（3.1・未決事項 #2）|
| DEFAULT パーティションに行が入っていないこと | **アプリケーション（未来日の打刻を拒む）+ 年次作業。** 監視は置かない（未決事項 #3）|
...
| # | 内容 | 判断の時期 |
| --- | --- | --- |
| 1 | ~~パーティションの自動生成~~ **解決済み。年次のマイグレーションで足す。`pg_partman` は採用しない。** 手順は [08_運用/年次作業.md](../../08_運用/年次作業.md) の 1 章にある（毎年 12 月に翌年ぶんを新しいマイグレーションとして足す。手で打たない） | 完了 |
| 2 | ~~打刻テーブルへの UPDATE / DELETE を DB 権限で禁止するか~~ **解決済み。DB 権限では守らない。** アプリとマイグレーションを**1 つのロールで動かす**と決めているので（[デプロイ手順書](../../08_運用/デプロイ手順書.md) 3 章）、同じロールに `REVOKE` しても意味がない。守るのは**ポートの形**である（`TimeClockEventRepository` は追記と参照しか持たない） | 完了 |
| 3 | ~~DEFAULT パーティションの監視~~ **解決済み。監視を置かない。行が入る経路を塞いだ。** 利用者が到達できたのは未来日の打刻だけで、これを 422 で拒むようにした（03 API設計書 未決 #2・IT-API-48）。残るのはパーティションを足し忘れた場合だけで、それは年次作業の手順で防ぐ。**入れる経路を塞ぐほうが、入ったことを検知するより確実である** | 完了 |
