# 社員・組織 API設計書

| 項目 | 内容 |
| --- | --- |
| 文書番号 | KNT-DES-103 |
| 版 | 0.3 |
| 対象パッケージ | `jp.co.sample.kintai.employee.presentation` |
| 関連文書 | [ドメインモデル設計書](ドメインモデル設計書.md) / [DB設計書](DB設計書.md) |
| 改訂 | 0.2（2026-09-01）設計レビューの指摘を反映<br>0.3（2026-09-01）レビュー第 2 回。承認者の導出理由を部署ごとの `reason` に細分化 |

---

## 1. 共通仕様

| 項目 | 仕様 |
| --- | --- |
| ベースパス | `/api` |
| 形式 | JSON（UTF-8） |
| 日付 | ISO 8601（`2026-04-01`） |
| 日時 | ISO 8601（`2026-04-01T09:00:00`）。**会社基準タイムゾーンの壁掛け時計時刻**。オフセットを含めない |
| 識別子 | UUID の文字列表現 |
| エラー | RFC 9457 (Problem Details) |
| 認証 | セッション（`JSESSIONID`）。未認証は 401 |

### 1.1 日時にオフセットを含めない理由

打刻や始業時刻は「会社の壁掛け時計が何時を指していたか」であり、
クライアントのタイムゾーンで解釈されるべきではない。オフセット付きで返すと、
ブラウザのタイムゾーン設定によって表示がずれる。

### 1.2 `date` パラメータの共通規則

多くのエンドポイントが基準日 `date` を受け取る。

| 規則 | 内容 |
| --- | --- |
| 既定値 | **`application` 層が `Clock` から解決した当日**。プレゼンテーション層で埋めない（AR-09） |
| 許容範囲 | 当日 + 1 年まで。これを超えると `400` |
| 未来日 | 許容する。発令済みの異動を事前に確認する運用があるため |

### 1.3 エラー応答

```json
{
  "type": "urn:kintai:error:duplicate-employee-number",
  "title": "社員番号が重複しています",
  "status": 409,
  "detail": "社員番号 E0001 は既に使用されています",
  "instance": "/api/employees",
  "errors": [ { "field": "employeeNumber", "message": "既に使用されています" } ]
}
```

| `type` | HTTP | 発生条件 |
| --- | --- | --- |
| `urn:kintai:error:resource-not-found` | 404 | 指定した社員・部署が存在しない |
| `urn:kintai:error:validation-failed` | 400 | 入力形式が不正。`errors` に項目ごとの詳細 |
| `urn:kintai:error:duplicate-employee-number` | 409 | 社員番号が在籍者と重複 |
| `urn:kintai:error:duplicate-email` | 409 | メールアドレスが在籍者と重複 |
| `urn:kintai:error:duplicate-department-code` | 409 | 部署コードが現存部署と重複 |
| `urn:kintai:error:overlapping-period` | 409 | 所属・部署長の期間が重複 |
| `urn:kintai:error:cyclic-department-hierarchy` | 422 | 部署階層に循環が生じる（自分自身または配下を親にした）|
| `urn:kintai:error:optimistic-lock-failure` | 409 | 他の利用者が先に更新した |
| `urn:kintai:error:business-rule-violation` | 422 | 形式は正しいが業務上受け付けられない |
| `urn:kintai:error:forbidden` | 403 | ロールまたは閲覧範囲の不足 |
| `urn:kintai:error:authentication-failed` | 401 | 社員番号かパスワードが違う。**どちらが違うかは区別しない。社員番号の形式違反もこれになる**（422 にすると理由を区別することになり、送った値も応答へ返る）|
| `urn:kintai:error:weak-password` | 422 | パスワードが規則を満たさない（設定・変更のとき）。**ログインでは使わない** |
| `urn:kintai:error:current-password-mismatch` | 422 | 現在のパスワードが違う（本人による変更のとき）|

**`conflict` のような粗い型にまとめない。**
フロントエンドが「どの項目を直せばよいか」を判断できなくなるため
（[アーキテクチャ設計書 6.2](../00_共通/アーキテクチャ設計書.md)）。

`errors` は `{ "field": string, "message": string }` の配列とする。

### 1.4 楽観ロック

更新系（`PATCH` / 一部の `POST`）はリクエストボディに `version` を必須で含める。

```json
{ "version": 3, "name": "山田 太郎" }
```

サーバが読んだ値で上書きすると、他の利用者の更新を黙って消してしまう。
不一致なら `409 optimistic-lock-failure` を返す。

---

## 2. エンドポイント一覧

| メソッド | パス | 概要 | 必要ロール |
| --- | --- | --- | --- |
| `POST` | `/api/sessions` | ログイン | **認証不要** |
| `DELETE` | `/api/sessions` | ログアウト | 認証のみ |
| `GET` | `/api/me` | ログイン中の社員の情報とロール | 認証のみ |
| `PUT` | `/api/me/password` | 自分のパスワードの変更 | 認証のみ |
| `PUT` | `/api/employees/{id}/password` | パスワードの初期発行・再設定 | `ADMIN` |
| `GET` | `/api/employees` | 社員一覧 | `EMPLOYEE`（範囲はロールに依存） |
| `GET` | `/api/employees/{id}` | 社員の詳細 | `EMPLOYEE`（範囲はロールに依存） |
| `POST` | `/api/employees` | 社員の登録 | `ADMIN` |
| `PATCH` | `/api/employees/{id}` | 社員情報の更新 | `ADMIN` |
| `PUT` | `/api/employees/{id}/roles` | ロールの付与・剥奪 | `ADMIN` |
| `POST` | `/api/employees/{id}/retirement` | 退職の登録 | `ADMIN` |
| `DELETE` | `/api/employees/{id}/retirement` | 退職の取消 | `ADMIN` |
| `GET` | `/api/employees/{id}/assignments` | 所属履歴 | 本人 / 上長 / `HR` / `ADMIN` |
| `POST` | `/api/employees/{id}/assignments` | 異動の登録 | `ADMIN` |
| `GET` | `/api/employees/{id}/monthly-attendances/{month}/approver` | 指定月の承認者（[05 が所有](../05_申請承認と締め/API設計書.md)）| 本人 / 上長 / `HR` / `ADMIN` |
| `GET` | `/api/departments` | 部署ツリー | `APPROVER` / `HR` / `ADMIN` |
| `POST` | `/api/departments` | 部署の登録 | `ADMIN` |
| `PATCH` | `/api/departments/{id}` | 部署の更新 | `ADMIN` |
| `POST` | `/api/departments/{id}/abolition` | 部署の廃止 | `ADMIN` |
| `POST` | `/api/departments/{id}/managerships` | 部署長の設定・交代 | `ADMIN` |

### 2.1 閲覧範囲（ロールによる差）

| ロール | 社員一覧の範囲 | 組織図 |
| --- | --- | --- |
| `EMPLOYEE` のみ | 自分自身のみ | **見られない**（自分の所属は `GET /api/me`、承認者は `GET .../monthly-attendances/{month}/approver` で取得） |
| `APPROVER` | 自分 + 自分が長を務める部署の配下すべて | 自分が長を務める部署以下 |
| `HR` / `ADMIN` | 全社員 | 全社 |

要件定義書 4.1 の閲覧範囲に従う。
**組織図の全体を一般社員へ公開しない。** 誰が誰の下にいるかは人事情報であり、
勤怠の記録・提出という業務に必要ないため。

> **`APPROVER` ロールは永続化しない。**
> 「承認者かどうか」の実体は `managerships`（その日に部署長を務めているか）である。
> ロールとして別途保持すると、「部署長だがロールが無く 403」「ロールはあるが対象 0 件」
> という不整合が起きる。認証時に `managerships` から導出して権限へ反映する。

**この判定は Spring Security のロール判定だけでは表現できない。**
「自分が長を務める部署の配下か」は組織の状態に依存する業務判断であり、
`application` 層が `OrganizationChart` を用いて行う。

> **承認者の照会は月の下に置く。**
> 第 1 版は `/api/employees/{id}/approver?month=` としていたが、
> **月に依存しない承認者は存在しない**（BR-11 の基準日は対象月初日であり、
> 異動すれば月ごとに変わる）。
> パスが月を含まないと「その社員の承認者」という月と無関係な概念があるように読め、
> 呼ぶ側が `month` を省いて当月の値を全期間へ当てる。
> 導出そのものを持つのは `approval.ApproverPolicy` なので、
> エンドポイントも 05 が所有する `monthly-attendances` の下に置く。

---

## 3. 主要なエンドポイントの詳細

### 3.1 `GET /api/me`

```json
{
  "id": "0195b000-0000-7000-8000-000000000001",
  "employeeNumber": "E0001",
  "name": "山田 太郎",
  "email": "yamada@example.com",
  "hiredOn": "2023-04-01",
  "roles": ["EMPLOYEE"],
  "department": {
    "id": "0195c000-0000-7000-8000-000000000003",
    "code": "S1A",
    "name": "第一営業課"
  }
}
```

> **労働時間制度（固定 / フレックス）はここに含めない。**
> 含めると `employee → workrule` というコンテキスト間依存図に無い依存が生まれ、
> ArchUnit の AR-06 に抵触する。フロントエンドは
> [`GET /api/work-rules/{seriesId}/effective`](../02_就業規則・カレンダー/API設計書.md) から
> 別途取得する。**API の都合でコンテキストの境界を崩さない。**

### 3.2 `GET /api/employees`

| クエリパラメータ | 型 | 既定 | 説明 |
| --- | --- | --- | --- |
| `date` | `date` | 当日 | この日付時点の所属を付与する |
| `departmentId` | `uuid` | — | 指定部署の配下に絞る |
| `includeRetired` | `boolean` | `false` | 退職者を含めるか |

```json
{
  "employees": [
    {
      "id": "0195b000-0000-7000-8000-000000000001",
      "employeeNumber": "E0001",
      "name": "山田 太郎",
      "email": "yamada@example.com",
      "hiredOn": "2023-04-01",
      "retiredOn": null,
      "department": { "id": "...", "code": "S1A", "name": "第一営業課" }
    }
  ]
}
```

| 決定 | 理由 |
| --- | --- |
| 配列を裸で返さずオブジェクトで包む | 一度 `[...]` で公開すると、後からページングのメタ情報を足せない |
| **所属を持たない社員も返す** | 未来日入社の社員が登録直後の一覧に現れないと、管理者が登録の成否を確認できない。`department` は `null` になる |
| `departmentId` 未指定なら所属で絞らない | 全社員一覧（`HR` / `ADMIN`）の用途 |

### 3.3 `POST /api/employees`

```json
{
  "employeeNumber": "E0004",
  "name": "鈴木 一郎",
  "email": "suzuki@example.com",
  "hiredOn": "2026-10-01",
  "departmentId": "0195c000-0000-7000-8000-000000000003",
  "additionalRoles": ["APPROVER"]
}
```

| 決定 | 理由 |
| --- | --- |
| 社員番号は手入力 | 人事は既存の採番体系を持つことが多い。一意性は DB の制約が保証する |
| 項目名を `additionalRoles` とする | **`EMPLOYEE` はサーバが無条件に付与する。** 要件定義書 4 章。指定できるのは追加ロールのみ |
| 登録と同時に所属を作る | 所属が無いと承認者が決まらず、勤怠を提出できない |

| 応答 | 条件 |
| --- | --- |
| `201 Created` | `Location` ヘッダに `/api/employees/{id}` |
| `400` | 入力形式が不正 |
| `409` | 社員番号またはメールアドレスが在籍者と重複 |
| `422` | 入社日より前の日付で所属を作ろうとした等 |

### 3.4 `PATCH /api/employees/{id}`

**更新可能な項目を限定する。**

| 項目 | 更新可否 | 理由 |
| --- | --- | --- |
| `name` | ○ | 改姓がある |
| `email` | ○ | 変更がある |
| `employeeNumber` | **×** | 認証 ID を兼ねるため。変更が必要なら退職＋再登録 |
| `hiredOn` | **×** | 所属の `valid_from` との整合が崩れる。誤登録は退職取消と併せて `ADMIN` が個別対応 |
| `retiredOn` | **×** | 副作用（所属・部署長のクローズ）があるため専用エンドポイントで行う |
| `roles` | **×** | `PUT /api/employees/{id}/roles` で行う |

`version` を必須とする（1.4）。

### 3.5 `POST /api/employees/{id}/retirement`（退職の登録）

```json
{ "retiredOn": "2026-09-30", "version": 3 }
```

**副作用を伴う。1 トランザクションで実行する。**

| 処理 | 内容 |
| --- | --- |
| 1 | `employees.retired_on` を設定 |
| 2 | 開いている `assignments` を **`retiredOn` の翌日**で閉じる |
| 3 | 開いている `managerships` を **`retiredOn` の翌日**で閉じる |

**`retiredOn` の翌日で閉じるのは、退職日当日は在籍しているため**である
（[ドメインモデル設計書 2.2](ドメインモデル設計書.md)）。
当日で閉じると、最終日の勤怠の承認者が導出できなくなる。

> 3 を忘れると、**その部署に所属する全社員の承認者が退職者になり続ける。**
> DB の制約では検出できない（既存行が後から不正になる種類の問題）ため、
> ここで確実に閉じる。

| 応答 | 条件 |
| --- | --- |
| `200 OK` | 登録成功。閉じた所属・部署長の件数を返す |
| `409` | 既に退職済み |
| `422` | 退職日が入社日より前 / 締め済みの月に遡る退職日 |

締め済みの月へ遡る退職は拒否する。確定済みの勤怠の承認者が変わってしまうため
（`approval` へ問い合わせて判定する）。

**退職の取消は版をクエリパラメータで受ける。**
RFC 9110 は `DELETE` の本文に意味を定めておらず、
中継するプロキシや HTTP クライアントに落とされることがある。
**版が届かないまま既定値で通ってしまう**のが最悪の壊れ方なので、本文に置かない。

`DELETE /api/employees/{id}/retirement?version=3` は退職の取消。
閉じた所属・部署長を再度開く。誤登録の訂正にのみ使う。

### 3.6 `POST /api/employees/{id}/assignments`（異動）

```json
{ "departmentId": "0195c000-0000-7000-8000-000000000002", "validFrom": "2026-10-01" }
```

現在の所属の `validTo` を `validFrom` で閉じ、新しい所属を開く。1 トランザクション。

| 応答 | 条件 |
| --- | --- |
| `201 Created` | 異動登録成功 |
| `409 overlapping-period` | 指定日以降に既に別の所属がある（遡及異動） |
| `422` | 入社日より前 / 廃止済みの部署 |

**遡及異動は認める。ただし締め済みの月へは遡れない。**
実務では発令漏れや日付誤りが起きるので訂正手段が要る。
拒むのは、確定済みの勤怠の承認者が後から変わる場合だけである。

判定は **`isClosedForAnyone`** を使う（IT-EMP-74〜77）。
対象社員 1 人の締めでは足りない ─
**その社員が部署長なら、部下全員の承認者が動く**（退職は部署長の在任も閉じる・落とし穴 78）。
同じ規則を、退職・退職の取消・部署長の任命・部署の廃止にも当てる。

**`409 overlapping-period` はアプリケーション層が返す。**
`assignments_no_overlap` でも弾かれるが、制約違反は利用者に説明できない
（落とし穴 66）。版 1.4 まではこの型を返す実装がどこにも無く、
**設計書だけが返すと言っていた**（落とし穴 108）。

### 3.7 `GET /api/employees/{id}/monthly-attendances/{month}/approver`

| クエリパラメータ | 型 | 必須 | 説明 |
| --- | --- | --- | --- |
| `month` | `YYYY-MM` | ○ | 対象月。BR-11 の基準日は月から導出する |

```json
{
  "kind": "INDIVIDUAL",
  "employeeId": "0195b000-0000-7000-8000-000000000002",
  "path": [
    { "departmentId": "0195c000-0000-7000-8000-000000000003",
      "departmentName": "第一営業課", "reason": "NO_MANAGER" },
    { "departmentId": "0195c000-0000-7000-8000-000000000002",
      "departmentName": "第一営業部", "reason": "NONE" }
  ]
}
```

> **氏名を返さない。** 承認者の氏名は `employee` が所有する概念なので、
> 画面が `employeeId` から引く（設計規約チェックリスト 3）。
> `month` と `basisDate` も返さない。どちらも利用者が送った経路から決まる値で、
> 返しても新しい事実が増えない。

`kind` は承認者の種類、`path` の各要素の `reason` は
**その部署でなぜ承認者が決まらなかったか**を示す。

| `kind` | 意味 |
| --- | --- |
| `INDIVIDUAL` | 個人が承認する |
| `HUMAN_RESOURCES` | **根まで遡っても得られず、人事担当が承認する**（`id` は `null`） |
| `NONE` | 対象月に所属が無い（入社前・退職後。この月には月次勤怠が存在しない） |

| `reason` | 意味 |
| --- | --- |
| `NONE` | **この部署で承認者が決まった** |
| `NO_MANAGER` | 長が未設定のため遡った |
| `SELF_APPROVAL_AVOIDED` | **部署長が本人だったため遡った** |
| `MANAGER_RETIRED` | 部署長が承認時点で退職済みのため遡った |
| `DEPARTMENT_ABOLISHED` | 部署が廃止済みのため遡った |

**理由を単一の値ではなく、部署ごとに返す。**
自部署は長が未設定・親では本人、のように経路上に複数の理由が混在する。
単一の `resolvedFrom` では「長が未設定なのか本人だったのか」を区別できても、
**どの部署でそれが起きたかを表せない。**

「なぜこの人が承認者なのか」は運用中に必ず問い合わせが来る。
**この経路そのものが、その問い合わせに対する答えになる。**

パラメータを `date` ではなく `month` にしているのは、
**BR-11 の基準日が「対象月の初日、ただし月中に所属開始があればその日」という
月に依存する規則だから**である。任意の日付を受け取ると規則を適用できない。

### 3.8 `GET /api/employees/{id}/assignments`（所属履歴）

```json
{
  "assignments": [
    { "departmentId": "...", "code": "S1",  "name": "第一営業部", "validFrom": "2025-04-01", "validTo": null },
    { "departmentId": "...", "code": "S1A", "name": "第一営業課", "validFrom": "2023-04-01", "validTo": "2025-04-01" }
  ]
}
```

**`validFrom` の降順（新しい順）** で返す。`validTo` が `null` は現在の所属を意味する。
半開区間なので、`validTo` の日には既に次の所属になっている。

### 3.9 `GET /api/departments`

部署をツリー構造で返す。**再帰 CTE で全件を 1 クエリで取得し、
アプリケーション側でツリーへ組み立てる。** 階層ごとにクエリを発行すると N+1 になる。

```json
{
  "departments": [
    { "id": "...", "code": "HQ", "name": "営業本部",
      "manager": { "id": "...", "name": "田中 一郎", "since": "2020-04-01" },
      "children": [
        { "id": "...", "code": "S1", "name": "第一営業部",
          "manager": { "id": "...", "name": "佐藤 花子", "since": "2021-04-01" },
          "children": [] }
      ] }
  ]
}
```

廃止済みの部署は既定で含めない（`includeAbolished=true` で含める）。

### 3.10 `PATCH /api/departments/{id}`

| 項目 | 更新可否 |
| --- | --- |
| `name` | ○ |
| `code` | ○（現存部署の間で一意） |
| `parentId` | ○ |
| `abolishedOn` | **×**（`POST /api/departments/{id}/abolition` で行う） |

> **親の変更は直列化する。**
> 循環検出トリガは他トランザクションの未コミットの変更を見ないため、
> 「A の親を C に」「C の親を A に」が同時に走ると循環が成立してしまう。
> `application` 層で `LOCK TABLE departments IN SHARE ROW EXCLUSIVE MODE` を取る。
> 部署の親変更は稀な操作なので、テーブルロックで十分。

### 3.11 `POST /api/departments/{id}/managerships`（部署長の設定）

```json
{ "employeeId": "0195b000-0000-7000-8000-000000000002", "validFrom": "2026-10-01" }
```

現任の期間を閉じて新しい期間を開く。
**期間の重複はアプリケーション層が `overlapping-period` で拒む**（IT-EMP-79）。
DB の排他制約は最後の防波堤として残すが、
制約違反は利用者に説明できない（落とし穴 66）。

| 応答 | 条件 |
| --- | --- |
| `201 Created` | 設定成功 |
| `409 overlapping-period` | 指定日以降に既に別の就任がある |
| `422` | 退職済みの社員 / 廃止済みの部署 |

**兼任を許容する。** 1 人が複数部署の長を兼ねることは制約しない（BR-11 補足）。

---

## 3.9 認証（`POST` / `DELETE /api/sessions`）

```
POST /api/sessions
{ "employeeNumber": "E0001", "password": "..." }
```

```json
{
  "id": "0195b000-0000-7000-8000-000000000001",
  "employeeNumber": "E0001",
  "name": "山田 太郎",
  "roles": ["EMPLOYEE", "APPROVER"]
}
```

| 応答 | 条件 |
| --- | --- |
| `200 OK` | 認証成功。`JSESSIONID` を発行する |
| `401 authentication-failed` | 社員番号かパスワードが違う。**どちらが違うかは返さない** |
| `401 authentication-failed` | 退職済みの社員 |

### 3.9.1 認証 ID は社員番号

メールアドレスは退職者の再割り当てと衝突する（要件定義書 7 章）。
社員番号は在籍者の中で一意であり、退職しても再利用しない。

### 3.9.2 失敗の理由を区別して返さない

「社員番号が存在しない」と「パスワードが違う」を区別して返すと、
**社員番号の総当たりで在籍者の一覧を作れる。**
退職済みも同じ応答にする。在籍しているかどうかを未認証の相手に教えない。

**存在しない社員番号でも、パスワードの照合を実行する。**
照合を飛ばすと応答時間の差から社員番号の存在が分かる（BCrypt は意図的に遅い）。

### 3.9.3 `APPROVER` は認証時に導出する

「承認者かどうか」の実体は `managerships`（その日に部署長を務めているか）である。
永続化されたロールに `APPROVER` は含めない（2.1 の注記）。

**導出は認証の時点で 1 度だけ行う。**
セッション中に部署長を交代しても、次のログインまで権限は変わらない。
リクエストのたびに引き直す設計にすると、
**同じ画面の中で操作の可否が変わる**という説明しづらい挙動になる。

### 3.9.4 セッションとログイン試行

| 項目 | 決定 | 理由 |
| --- | --- | --- |
| 形式 | セッション（`JSESSIONID`）。`HttpOnly` / `SameSite=Lax` | トークンを JavaScript から読めなくする。単一のフロントエンドしか持たないので JWT の利点が無い |
| ログイン時のセッション | **必ず作り直す**（`changeSessionId`） | セッション固定攻撃を防ぐ |
| CSRF | **有効**。`XSRF-TOKEN` クッキー + `X-XSRF-TOKEN` ヘッダ | Cookie でセッションを持つので、CSRF 対策を外せない。M1-a では API に認証が無かったため無効にしていた |
| 同時ログイン | 制限しない | PC とスマートフォンの併用を想定する |

---

## 3.10 パスワード（`PUT /api/me/password` / `PUT /api/employees/{id}/password`）

```
PUT /api/me/password
{ "currentPassword": "...", "newPassword": "..." }
```

```
PUT /api/employees/{id}/password        （ADMIN）
{ "newPassword": "..." }
```

| 応答 | 条件 |
| --- | --- |
| `204 No Content` | 変更成功 |
| `422 business-rule-violation` | 現在のパスワードが違う / 新しいパスワードが規則を満たさない |
| `403` | 他人のパスワードを `PUT /api/me/password` で変えようとした（そもそも到達しない） |

| 項目 | 決定 | 理由 |
| --- | --- | --- |
| 初期発行 | **`ADMIN` が設定して口頭・書面で伝える** | 100 名・単一事業所であり、メール送信基盤を持ち込む理由が無い |
| 再設定 | **`ADMIN` が再設定する。自己申告のリセットは設けない** | メールでのリセットは、退職者に再割り当てされたアドレスへ送る事故が起きる（要件定義書 7 章と同じ理由） |
| 強度 | 12 文字以上 72 バイト以下 | 下限は総当たりへの耐性。**上限は BCrypt が 72 バイトで切り捨てるため**。切り捨てを知らずに長い文を設定すると、実際にはその一部しか効いていない |
| 初回変更の強制 | **行わない** | `password_changed_at` は記録するが、強制の状態（`must_change`）を持たない。運用で足りる規模である |
| 保存 | BCrypt。DB の `CHECK` が `$2[aby]$` で始まる 60 文字以外を拒否する | 平文の誤保存を物理的に防ぐ |

**`ADMIN` による再設定で現在のパスワードを要求しない。**
本人が忘れた場合の救済経路なので、要求すると経路が成立しない。

---

### 3.10a 承認者の範囲は 1 か所（`ApproverScope`）

規則は 1 文である。

> 承認者は、その日に自分が長を務めている部署と、その配下の部署に所属する社員を見られる。

この規則には**向きの違う 2 つの問い方**があり、どちらも実際に必要である。

| 問い | 向き | 計算量 | 使う場所 |
| --- | --- | --- | --- |
| この 1 人を見てよいか | 対象の所属部署から根へ**上向き** | O(階層の深さ)＝高々 3 段 | 社員一覧（行ごとに呼ばれる）・勤怠の各コンテキスト |
| 見てよい部署はどれか | 長を務める部署から**下向き** | O(配下の部署数) | 組織図（SC-14） |

**1 つに畳まない。** 上向きだけにすると組織図が部署の数だけ再帰クエリを投げ、
下向きだけにすると社員一覧の 1 行ごとに配下すべてを展開する。

**畳まないかわりに、同じクラス（`ApproverScope`）に置いて 1 つのテストで縛る**
（`UT-AUTH-10`）。別々のクラスに持ったままだと、規則を変えたときに片方だけ直せてしまい、
「一覧には出るのに組織図に出ない部署」（あるいはその逆）が生まれる。
同じ検査が 2 か所にある状態を「守られている」と読まない（落とし穴 137）。

本人・`HR`・`ADMIN` の扱いはここには無い。
それは「承認者の範囲」ではなく閲覧の可否そのものなので、`EmployeeVisibility` の実装が持つ。

---

## 3.11 単体テストの観点（認証・閲覧範囲）

| ID | 観点 | 期待 | 要件 |
| --- | --- | --- | --- |
| UT-AUTH-01 | 本人 | いつでも見られる | 4.1 |
| UT-AUTH-02 | 一般社員が他人 | 見られない | 4.1 |
| UT-AUTH-03 | `HR` / `ADMIN` | 全社員を見られる | 4.1 |
| UT-AUTH-04 | 部署長が自部署の社員 | 見られる | 4.1 |
| UT-AUTH-05 | **上位部署の長が配下の部署の社員** | 見られる | 4.1 |
| UT-AUTH-06 | **別系統の部署の社員** | 見られない。ロールだけで判定すると通ってしまう | 4.1 |
| UT-AUTH-07 | `APPROVER` を持つが部署長でない | 見られない | 4.1 |
| UT-AUTH-08 | **就任前の日付** | 見られない。基準日で判定する | 4.1 |
| UT-AUTH-09 | **所属の無い社員** | 承認者からは見られない。人事は見られる | 4.1 |
| UT-AUTH-10 | **上向き（1 人を見てよいか）と下向き（見てよい部署はどれか）** | どの (承認者, 社員) の組でも同じ答えになる。**同じ規則の 2 つの向きなので、片方だけ直せる状態にしない** | 4.1 |
| UT-AUTH-11 | **一致の検査そのもの** | 恒真でない。真になる組と偽になる組が実際にある（どちらも常に偽でも一致はする）| 4.1 |
| UT-PWD-01 | 11 文字のパスワード | 生成時に例外（BR-13） | BR-13 |
| UT-PWD-02 | **UTF-8 で 73 バイトのパスワード** | 生成時に例外。BCrypt の切り捨てを黙って受け入れない | BR-13 |
| UT-PWD-03 | **BCrypt の形をしていないハッシュ** | 生成時に例外。平文を型に入れられない | BR-13 |
| UT-PWD-04 | `toString()` | 値を出さない | BR-13 |

---

## 3.12 API の結合テストの観点（社員名簿）

API から DB までを通す。**閲覧範囲は「見える」と「見えない」の両方を検証する。**
画面が隠していても、API が返せば漏洩する。

| ID | 観点 | 期待 | 対応要件 |
| --- | --- | --- | --- |
| IT-EMP-27 | **一般社員の一覧** | 自分だけ。組織図は人事情報であり勤怠の記録に要らない | 4.1 |
| IT-EMP-28 | **承認者の一覧** | 自分と、自分が長を務める部署の配下だけ | 4.1 |
| IT-EMP-29 | 人事・管理者の一覧 | 全社員 | 4.1 |
| IT-EMP-30 | **範囲外の社員の詳細** | 403。一覧だけ絞っても詳細から漏れる | 4.1 |
| IT-EMP-31 | 未認証で一覧 | 401 | 要件 4 |
| IT-EMP-32 | `departmentId` で絞る | その部署の配下だけが返る | 3.2 |
| IT-EMP-33 | **未来日入社の社員** | 一覧に現れ、`department` は `null`。登録の成否を確認できる | 3.2 |
| IT-EMP-34 | `includeRetired` | 既定では退職者を含めず、指定すると含む | 3.2 |
| IT-EMP-35 | **社員の登録** | 201。`Location` を返し、**所属も同時に作られる** | 3.3 |
| IT-EMP-36 | 追加ロールの指定 | `EMPLOYEE` と併せて付く | 3.3 |
| IT-EMP-37 | **社員番号が在籍者と重複** | 409 `duplicate-employee-number`。一意制約違反にしない | 3.3 |
| IT-EMP-73 | **退職日が未来の社員の番号を再利用** | 409。まだ在籍しているので渡せない。部分一意インデックスは `retired_on IS NULL` の行しか見ないので DB も止められない | 3.3 |
| IT-EMP-74 | **締め済みの月へ遡って部署長を任命** | 409 `month-already-closed`。部署長が変われば部署全員の承認者が変わる | 3.9 |
| IT-EMP-75 | **締め済みの月に部署を廃止** | 409 `month-already-closed`。廃止は承認者の遡りの経路を変える | 3.9 |
| IT-EMP-76 | **退職月が締め済みのときの退職の取消** | 409 `month-already-closed`。退職の登録と対称に守る | 3.8 |
| IT-EMP-77 | **所属者がいる部署の廃止** | 409 `department-has-members`。所属行が廃止済みの部署を指したまま残る | 3.9 |
| IT-EMP-78 | **指定日以降に別の所属がある遡及異動** | 409 `overlapping-period`。制約違反にしない | 3.6 |
| IT-EMP-79 | **指定日以降に別の就任がある任命** | 409 `overlapping-period`。同上 | 3.9 |
| IT-EMP-38 | **大文字違いの同一メール** | 409 `duplicate-email` | 3.3 |
| IT-EMP-39 | 管理者でない利用者の登録 | 403 | 3.3 |
| IT-EMP-40 | 存在しない部署を指定 | 404 | 3.3 |
| IT-EMP-41 | **氏名とメールの更新** | 200。社員番号と入社日は変わらず、版が進む | 3.4 |
| IT-EMP-42 | **古い版での更新** | 409 `optimistic-lock-failure`。値は変わらない | 1.4 |
| IT-EMP-43 | 版を含まない更新 | 400 | 1.4 |

### 異動・退職・ロール

| ID | 観点 | 期待 | 対応要件 |
| --- | --- | --- | --- |
| IT-EMP-44 | **異動** | 現在の所属が閉じ、新しい所属が開く。履歴は新しい順 | 3.6 / 3.8 |
| IT-EMP-45 | 入社日より前への異動 | 422 `before-hire-date` | 3.6 |
| IT-EMP-46 | 廃止済みの部署への異動 | 422 `department-abolished` | 3.6 |
| IT-EMP-47 | 管理者でない利用者の異動 | 403 | 3.6 |
| IT-EMP-48 | **退職の登録** | 所属が<strong>退職日の翌日</strong>で閉じる | 3.5 |
| IT-EMP-49 | **部署長を務めていた社員の退職** | 部署長も閉じる。退職日当日は在籍、翌日から空く | 3.5 |
| IT-EMP-50 | 二重の退職登録 | 409 `already-retired` | 3.5 |
| IT-EMP-51 | 退職日が入社日より前 | 422 | 3.5 |
| IT-EMP-52 | **退職の取消** | 閉じた所属と部署長が開き直る | 3.5 |
| IT-EMP-53 | 退職していない社員の取消 | 409 `not-retired` | 3.5 |
| IT-EMP-54 | **退職の取消と過去の異動** | 異動で閉じた所属は巻き戻さない。閉じた日で絞る | 3.5 |
| IT-EMP-55 | ロールの付与 | `EMPLOYEE` と併せて付く | 3.4 |
| IT-EMP-56 | **ロールの置き換え** | 送った集合が全体になる。`EMPLOYEE` は残る | 3.4 |
| IT-EMP-57 | **`APPROVER` の付与** | 422 `not-assignable-role`。実体は部署長を務めている事実 | 2.1 |
| IT-EMP-58 | 管理者でない利用者のロール変更 | 403 | 3.4 |

### 部署ツリー

| ID | 観点 | 期待 | 対応要件 |
| --- | --- | --- | --- |
| IT-EMP-59 | **全社のツリー** | 階層つきで返り、部署長は就任日つき。長が未設定なら `manager` は `null` | 3.9 |
| IT-EMP-60 | **一般社員が組織図を見る** | 403。誰が誰の下にいるかは人事情報であり、勤怠の記録に要らない | 4.1 |
| IT-EMP-61 | **承認者のツリー** | 自分が長を務める部署以下だけ。親が対象外なので自部署が根になる | 4.1 |
| IT-EMP-62 | 長を務めていない承認者 | 403 | 4.1 |
| IT-EMP-63 | 廃止済みの部署 | 既定で含まれない。`includeAbolished` で含む | 3.9 |
| IT-EMP-64 | 部署の登録 | 201 | 3.9 |
| IT-EMP-65 | 部署コードが現存部署と重複 | 409 `duplicate-department-code` | 1.3 |
| IT-EMP-66 | **廃止済みの部署コード** | 再利用できる | 1.3 |
| IT-EMP-67 | 名称と親の更新 | 200。ツリーの位置が変わる | 3.10 |
| IT-EMP-68 | **親に自分の配下を指定** | 422 `cyclic-department-hierarchy` | 3.10 |
| IT-EMP-69 | 管理者でない利用者の登録 | 403 | 3.9 |
| IT-EMP-70 | **配下に現存する部署があるまま廃止** | 422 `department-has-children`。葉から順なら通る | 3.9 |
| IT-EMP-71 | 部署長の交代 | 現任の期間が閉じ、交代日から新任になる | 3.11 |
| IT-EMP-72 | 退職済みの社員を部署長に | 422 `retired-employee` | 3.11 |

**循環は業務エラーとして返す。**
DB の `departments_reject_cycle_trigger` も同じことを守っているが、
PL/pgSQL の `RAISE` は制約違反ではないので Problem Details に写らず、
**親の選び間違いが 500 になる。**
テーブルロックを取ったあとに `application` 層で検査し、トリガは最後の防波堤として残す。

**配下に現存する部署があるうちは廃止できない。**
親だけを廃止すると、子が親のいない部署として残り、
承認者の遡りが根へ到達できなくなる。

**退職で部署長を閉じ忘れると、その部署に所属する全社員の承認者が退職者になり続ける。**
既存の行が後から不正になる種類の問題なので、DB の制約では検出できない。
IT-EMP-49 が守る。

**退職の取消は「閉じた日」で絞って開き直す。**
社員 ID だけで開き直すと、異動で閉じた過去の所属まで開いてしまい、
期間が重なって排他制約に弾かれる。IT-EMP-54 が守る。

**一覧に `version` を載せない。**
行ごとに引くと社員数ぶんの問い合わせと認可判定が重複する。
版が要るのは更新する 1 人だけなので、詳細でだけ返す。

**名簿の一覧は「その日に在籍していたか」では絞らない。**
未来日入社の社員は入社日がまだ来ていないので、
在籍判定（`findActiveOn`）で引くと一覧に現れない。
名簿は退職者を除くかどうかだけで絞る（`findForDirectory`）。
**2 つは別の問いであり、1 つのメソッドにまとめない。**

---

## 4. 実装上の注意

| # | 内容 |
| --- | --- |
| 1 | **DB の制約違反を握り潰さない。** `DataIntegrityViolationException` の制約名から 1.3 の `type` へ変換する。「登録に失敗しました」で終わらせない |
| 2 | レスポンスの `record` はドメインの型を直接公開しない。ドメインの構造変更が API の互換性を壊さないようにする |
| 3 | 閲覧範囲の絞り込みは `application` 層で行う。プレゼンテーション層に業務判断を置かない |
| 4 | `date` の既定値は `application` 層が `Clock` から解決する。コントローラで `LocalDate.now()` を呼ばない（AR-09） |
| 5 | 退職・異動・部署長交代は複数テーブルを更新する。`@Transactional` の範囲がユースケース全体を覆っていることをテストで確認する |

---

## 5. 未決事項

| # | 内容 | 判断の時期 |
| --- | --- | --- |
| 1 | ~~社員一覧にページングを導入するか~~ **解決済み。導入しない。** 対象は 100 名（要件 2.2）で全件返す。閲覧範囲の絞り込みはアプリ側で行うので、ページングを入れると絞り込み後の件数が読めなくなる。将来のために応答をオブジェクトで包む形だけは確保してある | 完了 |
| 2 | ~~OpenAPI の定義から TypeScript の型を自動生成するか~~ **解決済み。自動生成しない。手書き。** springdoc は依存に入っていない。手書きの型はコンパイラに守られない（落とし穴 143）ので、実物のバックエンドへ当てる通しで担保する（CLAUDE.md 未決 #3 と同じ決定） | 完了 |
| 3 | ~~遡及異動を許可する場合の締め済み月との整合~~ **解決済み。遡及異動を許可し、締め済みの月への遡及を `month-already-closed`（409）で拒む**（ドメインモデル設計書 7 章 #2）。**`overlapping-period` は返らない**（この型を返すのは就業規則だけである） | 完了 |
