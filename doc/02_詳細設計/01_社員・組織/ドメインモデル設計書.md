...
public record Employee(
        ...
        Email email,
        LocalDate hiredOn,
        Optional<LocalDate> retiredOn,   // 空は在籍中。null は許さない
        Set<Role> roles) {

    public Employee {
        // 退職日 < 入社日 は人事が入力で踏める誤りなので業務エラー
        if (retiredOn.filter(r -> r.isBefore(hiredOn)).isPresent()) {
...
public record Department(
        ...
        DepartmentCode code,
        String name,
        Optional<DepartmentId> parentId,    // 空はルート
        Optional<LocalDate> abolishedOn) {

    public Department {
        // 1 段の循環はここで防ぐ。多段は DB のトリガが防ぐ（CHECK に再帰は書けない）
        if (parentId.filter(id::equals).isPresent()) { throw new IllegalArgumentException(...); }
    }

    public boolean isRoot() { return parentId.isEmpty(); }

    /** 廃止日は「その日から使えない」上限。退職日（最終在籍日）とは向きが違う。 */
    public boolean isActiveOn(LocalDate date) {
...
既定実装 `RepositoryBackedOrganizationChart` は 4 つのポートを組み合わせるだけで、
フレームワークに依存しないので `domain` に置く。Bean の定義は
`employee.infrastructure` が行う（AR-01：ドメインは Spring を知らない）。
...
| UT-EMP-05 | 所属している日の部署 | その部署を返す | — |
| UT-EMP-06 | 所属が無い日の部署 | 空を返す（例外にしない） | — |
| UT-EMP-07 | 異動の前後 | 半開区間なので、異動日当日は新しい部署 | 2.2 |
| UT-EMP-08 | 祖先の列挙 | 自分自身から根へ向かう順で返す | BR-11 の 4 |
| UT-EMP-09 | 部署長の交代 | 基準日時点の長を返す | BR-11 の 3 |
| UT-EMP-10 | 月の途中で所属開始 | その開始日を返す（BR-11 の 1 の基準日） | BR-11 の 1 |
| UT-EMP-11 | 月初日に所属開始 | **空を返す。** 「月の途中で始まった」ではないので基準日は月初日でよい | BR-11 の 1 |
...
| # | 内容 | 判断の時期 |
| --- | --- | --- |
| 1 | ~~人事担当が複数いる場合、誰が承認者になるか~~ **解決済み。`HR` ロールを持つ全員。ただし対象社員本人を除く**（[05 の 3.2](../05_申請承認と締め/ドメインモデル設計書.md)）。特定の 1 人には固定しない。**人事担当を 2 名以上置くことが運用上の前提**で、1 名しかいないとその 1 名の月を誰も決裁できない | 完了 |
| 2 | ~~異動・退職の遡及訂正をどこまで許すか~~ **解決済み。遡及訂正は認める。ただし対象日の属する月が締め済みなら拒む。** 判定は **`isClosedForAnyone`**（社員 1 人の締めでは足りない。その社員が部署長なら部下全員の承認者が動く）。**退職の取消**も同じ規則で守る（IT-EMP-76） | 完了 |
| 3 | ~~部署廃止時に配下の所属をどう閉じるか~~ **解決済み。閉じない。所属者が残っていれば廃止を拒む**（`department-has-members`・IT-EMP-77）。自動で閉じると所属の無い社員が生まれ、承認者が決まらなくなる（落とし穴 26）。先に異動させてから廃止する。**配下の部署だけを見ていたので、所属者は素通りしていた** | 完了 |
