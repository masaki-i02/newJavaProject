...
```java
public record Employee(..., LocalDate hiredOn, LocalDate retiredOn, ...) {

    /** 在籍期間。退職日は最終在籍日なので、半開区間へは翌日を渡す。 */
    public DateRange activePeriod() {
        return new DateRange(hiredOn, retiredOn == null ? null : retiredOn.plusDays(1));
    }
...
`approval` の `ApproverPolicy` は、この 3 つを組み合わせて BR-11 を適用する。

```java
// approval/domain --- BR-11 の 4 と 5 はここで適用される
public Approver resolve(EmployeeId target, YearMonth month) {
    LocalDate basis = basisDateFor(target, month);              // BR-11 の 1
    Optional<Department> own = organizationChart.departmentOf(target, basis);   // BR-11 の 2
    for (Department department : organizationChart.selfAndAncestorsOf(own.id())) {
        ...
        if (!employees.findById(...).isActiveOn(today)) continue; // 承認時点で退職済み
        return Approver.of(manager.get(), resolvedFrom(department, own));
    }
    return Approver.humanResourcesFallback();                   // BR-11 の 5
}
```
...
呼び出し側が「いつから就任している長か」を必要とするため（API の `resolvedFrom` の詳細化）。
