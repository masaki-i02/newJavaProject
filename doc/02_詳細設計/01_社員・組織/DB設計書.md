...
| アカウントロックの列を持たない | **要件が「設けない」と決めている**（BR-13）。認証 ID が社員番号なので、ロックを設けると**社員番号を知っている者が任意の社員を締め出せる**。総当たりへの耐性は長さの下限と BCrypt の計算時間で確保し、気づく手段は認証の記録に持たせる |
...
| # | 内容 | 判断の時期 |
| --- | --- | --- |
| 1 | ~~認証時に退職者を除外する具体的な実装~~ **解決済み。SQL では絞らない。** 社員番号で引いた行を**すべて**返し、`Employee.isActiveOn(today)` で絞る。**`WHERE retired_on IS NULL` と書き直してはいけない**（退職を先の日付で登録した社員はまだ在籍しているのに退職日を持つ）。1 件で受けると、退職者の番号を再割り当てした瞬間に `IncorrectResultSizeDataAccessException` になり、**その番号では誰もログインできなくなる**（IT-AUTH-23）。在籍者が 2 人いる場合は理由を記録して認証を通さない | 完了 |
