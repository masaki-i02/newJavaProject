...
| IT-WR-23 | **2026 年 5 月・所定 21 日・1 日 8 時間** | 4.4 が `exceeds_limit = false`（10,080 ≤ 10,628）を返す。**真になるケースだけでは判定の向きを確かめられない** | 済 |
...
| # | 内容 | 判断の時期 |
| --- | --- | --- |
| 1 | ~~祝日データの投入方法~~ **解決済み。人事が `POST /api/calendars/bulk` で登録する。CSV の取込は設けない。** 内閣府の CSV は「国民の祝日」しか持たず、年末年始・創立記念日・所定休日は結局手で入れる。しかも BR-18 の分母は**年度の全日が登録されていること**を前提にするので、外部データの取り込みの成否に依存させられない | 完了 |
| 2 | ~~就業規則の改定時に過去分を再計算するか~~ **解決済み。自動では再計算しない。人事が明示的に指示する**（`POST .../recalculation`）。締め済みの月は改定そのものを拒み、月次清算に効く値の月中改定も拒む（API設計書 2.2） | 完了 |
| 3 | ~~系列の版に隙間ができたことを検出する定期チェックを設けるか~~ **解決済み。定期チェックは置かない。隙間を作る経路を塞いだ。** 改定は必ず連続する（現行版を閉じて同じ日から開く）ので、隙間を作れたのは**適用**の側だった。その日に有効な版を持たない系列への適用を 422 で拒む（`no-effective-work-rule-version`・IT-WR-45）。**入れる経路を塞ぐほうが、入ったことを検知するより確実である。** 経路外から作られた隙間は月次清算が `work-rule-not-assigned` で拒む（IT-API-45） | 完了 |
