...
public record FlextimeSystem(
     ...
     * コアタイムが外枠の先頭から始まる規則では帯が存在しないので空を返す。
     */
    ...
    /** コアタイム後のフレキシブル帯（15:00 – 22:00）。同様に、無ければ空。 */
...
>
> **用途は 2 つある。** 所定総労働時間の算出（`attendance` の月次清算）と、
> **割増賃金の基礎額の分母**（労基則 19 条 1 項 4 号。BR-18）である。
> 後者は年間の所定 = Σ（各清算期間の所定総）として使うので、
> 係数としての用途から外れていない。
> 取り出す口は `WorkRule.scheduledDailyWorkingTime()`（`switch` は 1 か所）。

#### 年間の所定は「日ごと」に足す（BR-18）

割増賃金の基礎額の分母を求めるとき、
**系列ごとに 1 年ぶんを足してから比べてはならない。**

系列の一覧だけを取り、系列ごとに年度の全所定労働日について版を要求すると、
**年度の途中で新設した系列**（10 月からフレックスを導入した、など）が
4 月〜9 月に版を持たないことを「隙間」と誤診断し、
**その年度のどの月も永久に出力できなくなる**（落とし穴 131）。

`findUsagesIn(period)` が返すのは系列だけでなく**適用期間**である。
日ごとに「その日に適用されている系列」を集め、

| 状態 | 応答 |
| --- | --- |
| 1 つに定まる | その値を足す |
| 2 つ以上あって値が違う | `multiple-scheduled-working-times`（422）|
| 1 つも無い（所定労働日なのに）| `work-rule-not-applied-on`（422）|
| 適用されているのに版が無い | `work-rule-version-missing`（422）|

副産物として、**年度の途中で全社の所定を改定した年度**も正しく数えられる。
...
だから**版が月中で切り替わってよいかどうかは、
切り替わった先が月次に効くかどうかで決まる。**

| 項目 | 月次清算に効くか |
| --- | --- |
| `workingTimeSystem`（所定・コアタイム） | **効く**（所定総労働時間・不足時間・コアタイム不在）|
| `statutoryDailyWorkingTime` / `statutoryWeeklyWorkingTime` | **効く**（法定労働時間の総枠・通算・週 40 時間超）|
| `nightWindow` | 効かない。日次の割増区分にしか使わない |
| `premiumRates` | 効かない。率は給与側が持つ（BR-18）|

```java
/** 月次清算に効く部分が同じか。効かない項目を潰した版どうしを比べる。 */
public boolean hasSameMonthlyBasisAs(WorkRule other) { ... }
```

**項目名を並べた比較にしない。**
`WorkRule` に項目が増えて月次が使い始めたとき、名前を並べた実装は気づけない。
**効かない項目を既定値で潰した `WorkRule` を組み立てて比べる**と、
項目が増えたときに正準コンストラクタの引数が増えてコンパイルが落ち、
足した人がどちらなのかを決めることになる（CLAUDE.md 落とし穴 87）。
...
| 小数第 3 位に正規化して保持する | `record` の `equals` は `BigDecimal.equals` を使い、値だけでなく**スケール**も比較する。DB の `NUMERIC(4,3)` は `0.250` を返すため、正規化しないと定数 `0.25` と等しくならず、**読み書きを往復しただけで別の値になる** |
...
**`record` ではなく `enum` にする。**
深夜帯を自由に狭められると、割増率の下限だけ守っても
**割増の対象になる時間そのものが消える**（CLAUDE.md 落とし穴 15）。
`record` に「許可した 2 つだけを通す」検証を書いても、
型の上では任意の時刻を渡せるのに実行時にしか弾けない。
取りうる値が法で 2 つに固定されているなら列挙にして、
**不正な値をそもそも書けなくする**（CLAUDE.md 4.3）。
...
**日数の数え方はインタフェースの既定実装に置く。**
実装側に置くと、半開区間の数え方（末日を含むか）を実装の数だけ間違えられる。
テストの代役に置いた場合はさらに悪く、**そのテストは本番のコードを 1 行も検査しない**
（CLAUDE.md 落とし穴 37）。代役が答えるのは「その日が何の日か」だけにする。

端が番兵（無期限）の期間では数えられないので例外にする。
`LocalDate.MAX` まで 1 日ずつ回すと約 21 億回になり、応答が返らない。
...
public interface CompanyCalendar {
    ...
    /**
     * 指定期間の所定労働日数。半開区間で受ける。
     * 数え方は dayTypeOf から一意に決まるので、既定実装をここに置く。
     */
...
**期間を引数で受け取らない。**`SettlementPeriod` のメソッドなので、
対象の期間は自分自身が持っている。引数で受けられるようにすると、
暦月を渡す誤り（3.2 が防ごうとしているまさにその誤り）が型で防げなくなる。
警告が持つのも `DateRange` ではなく `YearMonth` である。人事に知らせるのは
「どの月の設定が問題か」であって、期間の端点ではない。
...
| UT-WR-09 | **深夜帯の取りうる値** | 22:00–05:00 と 23:00–06:00 の 2 つしか存在しない（それ以外は `enum` なのでコンパイルできない） | BR-06 |
...
| UT-WR-20 | **フレックスで所定が法定を超える規則** | 生成時に業務エラー。固定時間制だけでは、所定を取り出す `switch` のフレックスの枝が検査されない | BR-04 |
| UT-WR-21 | **所定ちょうど 6 時間・休憩 0 分** | 登録できる。34 条は「6 時間を**超える**場合」 | BR-08 |
| UT-WR-22 | **所定 6 時間 1 分・休憩 44 分** | 業務エラー | BR-08 |
| UT-WR-23 | **所定ちょうど 8 時間・休憩 45 分** | 登録できる。34 条は「8 時間を**超える**場合」 | BR-08 |
| UT-WR-24 | **所定 8 時間 1 分・休憩 45 分** | 業務エラー。休憩 60 分が要る | BR-08 |
| UT-WR-25 | **所定 10 時間 15 分・休憩 45 分** | 業務エラー。`FixedTimeSystem` 単体でも作れない | BR-08 |
| UT-WR-26 | **深夜帯だけが違う 2 つの版** | 月次清算に効く部分は同じ。月中の改定を許す根拠 | BR-05 |
| UT-WR-27 | **所定労働時間が違う 2 つの版** | 月次清算に効く部分が違う | BR-05 |
| UT-WR-28 | **1 日の法定労働時間が違う 2 つの版** | 月次清算に効く部分が違う。総枠（`statutoryTotalLimit`）に効く | BR-05 |
| UT-CAL-01 | 未登録日の区分 | `WORKDAY` を返す | BR-07 |
| UT-CAL-02 | 所定労働日数 | 土日祝を除いた日数を返す | BR-05 |
| UT-CAL-03 | **半開区間の端** | 末日は数え、その翌日は数えない。**両端を休日にしない**（どちらの変異にも反応しなくなる） | BR-05 |
| UT-CAL-04 | **無期限の期間の所定労働日数** | 例外。番兵まで 1 日ずつ回すと約 21 億回になる | — |
...
| # | 内容 | 判断の時期 |
| --- | --- | --- |
| 1 | ~~祝日データを内閣府の CSV から取り込むか~~ **解決済み。人事が手動登録する**（`POST /api/calendars/bulk`）。年度の全日が登録されていることが BR-18 の分母の前提なので、外部データの取り込みの成否に依存させられない | 完了 |
| 2 | ~~改定時、過去分の日次勤怠を自動で再計算するか~~ **解決済み。自動では再計算しない。人事が明示的に指示する**（`POST .../recalculation` の 2 本）。改定そのものは締め済みの月を拒み、月次清算に効く値の月中改定も拒む | 完了 |
| 3 | ~~フレックスの清算期間を 1 か月以外に広げるか~~ **解決済み。広げない。1 か月に固定する。** `SettlementPeriod` は `YearMonth` を持ち、`monthly_settlements` は `(employee_id, target_month)` で一意である。広げるには**労基法 32 条の 3 第 2 項の「1 か月ごとに週平均 50 時間」という別の枠**が新たに要り、その概念はドメインにもテストにも 1 件も無い。ADR 0008（シフト）と同型で、複数の BR を同時に作り直す変更になる | 完了 |
