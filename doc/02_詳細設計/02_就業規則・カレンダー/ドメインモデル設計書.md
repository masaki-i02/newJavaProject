...
public record WorkRule(
    ...
    public WorkRule {
        requireAtMost(statutoryDailyWorkingTime, Duration.ofHours(8), "1 日の法定労働時間");
        requireAtMost(statutoryWeeklyWorkingTime, Duration.ofHours(40), "1 週の法定労働時間");
...
public record NightWindow(LocalTime start, LocalTime end) {   // 22:00 – 05:00
    ...
    private static final Set<NightWindow> LEGAL = Set.of(
            new NightWindow(LocalTime.of(22, 0), LocalTime.of(5, 0)),   // 原則
            new NightWindow(LocalTime.of(23, 0), LocalTime.of(6, 0)));  // 厚生労働大臣が定める地域

    public NightWindow {
        // 深夜帯を自由に狭められると、深夜割増を実質的に無効化できてしまう
        if (!LEGAL.contains(new NightWindow(start, end))) {
...
```java
/** 清算期間 = 暦月 ∩ 在籍期間。半開区間どうしの交差。 */
public DateRange settlementPeriod(YearMonth month, DateRange employmentPeriod) {
    DateRange calendarMonth = new DateRange(month.atDay(1), month.plusMonths(1).atDay(1));
    return calendarMonth.intersect(employmentPeriod);
}
```
...
```java
/** 清算期間の法定労働時間の総枠。= 暦日数 ÷ 7 × 週法定労働時間 */
public Duration statutoryTotalLimit(DateRange period, Duration statutoryWeekly) {
    long days = period.days();
    return Duration.ofMinutes(days * statutoryWeekly.toMinutes() / 7);   // 切り捨て
}
```
...
| UT-WR-09 | **深夜帯を 02:00–03:00 で生成** | 生成時に例外 |
| UT-WR-10 | **1 日の法定労働時間を 12 時間で生成** | 生成時に例外 |
| UT-WR-11 | **所定 9 時間の固定時間制を生成** | 生成時に例外（所定 > 法定） |
| UT-WR-12 | **所定 7 時間・休憩 30 分を生成** | 生成時に例外（労基法 34 条） |
| UT-WR-13 | **改定後の日付で `findEffective`** | 新しい版が返る。適用行は書き換えていない |
| UT-WR-14 | **改定前の日付で `findEffective`** | 古い版が返る |
| UT-WR-15 | **版に隙間がある日付で `findEffective`** | 空が返る（例外にしない） |
| UT-WR-16 | **4/15 入社の清算期間** | `[2026-04-15, 2026-05-01)` |
| UT-WR-17 | **9/20 退職の清算期間** | `[2026-09-01, 2026-09-21)` |
| UT-WR-18 | **2026-06・所定 22 日・8 時間の総枠検証** | 10,560 > 10,285 で警告が返る |
| UT-WR-19 | **総枠を超えない月の総枠検証** | 警告は返らない |
| UT-CAL-01 | 未登録日の区分 | `WORKDAY` を返す |
| UT-CAL-02 | 所定労働日数 | 土日祝を除いた日数を返す |
| UT-CAL-03 | **半開区間の端** | `[6/1, 7/1)` は 6/30 を含み 7/1 を含まない |
