...
> **`bootRun` を 2 つ起動しない。** 2 つ目は
> 「Port 8080 was already in use」で**静かに死に**、シナリオは
> **古いインスタンス**に当たり続ける。直前に変異テストを流していると、
> 生きている側は変異したクラスを読み込んでいるので、
> **製品の欠陥と見分けがつかない形で落ちる**（CLAUDE.md 落とし穴 165）。
> 起動する前に `curl -sf http://localhost:8080/actuator/health` で
> ポートが空いていることを確かめる。
